...
- **Tools are routable via `arete route` and `arete skill route` (2026-02-22, free functions 2026-04-30)**: Previously only skills from `.agents/skills/` were in the routing candidate pool. Tools in `.cursor/tools/` (or `.claude/tools/`) were invisible to the router, even though `SkillCandidate` already had `type: 'tool'` support. Fixed by adding tool discovery in `@arete/core` and merging tool candidates into the routing pipeline. The tool-to-candidate mapping includes all scoring-relevant fields (`triggers`, `description`, `work_type`, `category`). **Lesson**: If a TOOL.md is missing `triggers` in its frontmatter, the tool will never score > 0 and won't be routable. Always include `triggers` when creating new tools. Discovery API: `listTools(storage, toolsDir)` / `getTool(storage, id, toolsDir)` from `@arete/core` (originally a `ToolService` class).
