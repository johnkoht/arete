...
2. **L2: Items** (`.arete/memory/items/`) — Atomic facts (decisions, learnings, observations); user-approved, citable
3. **L3: Computed views** — three complementary surfaces, all auto-regenerated by `arete memory refresh`:
   - **Areas** (`.arete/memory/areas/{slug}.md`) — operational snapshot per work domain (active people, open work, topic index)
   - **Topics** (`.arete/memory/topics/{slug}.md`) — encyclopedic wiki per topic; LLM-synthesized narrative that accumulates with every meeting ingest. **Source of CLAUDE.md's Active Topics boot context** — when you open a /guide session, turn 1 already has the topic list in attention.
   - **Summaries** (`.arete/memory/summaries/`) — free-form synthesis (collaboration profile, sessions)

Plus navigation surfaces:
- `.arete/memory/index.md` — catalog of topic + person + area pages (Obsidian landing)
- `.arete/memory/log.md` — append-only audit of ingest/refresh/lint events

**When memory is updated**:
- `process-meetings` + `arete meeting apply` — extract decisions/learnings AND propose topics for the meeting; topics get aliased against existing slugs, stubs created for new ones, committed items trigger topic-page narrative integration via the LLM
...
**Topic retrieval for skills**: `arete topic find <query> --json` returns top-k relevant topic pages with budget-truncated `bodyForContext`. See `topic_page_retrieval` pattern in `.agents/skills/PATTERNS.md`.
