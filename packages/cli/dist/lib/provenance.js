 ...
 */
export function classifyProvenance(path) {
    const norm = path.replace(/\\/g, '/');
    const m = PROJECT_SUBPATH.exec(norm);
    if (m === null)
        return undefined;
    const rest = m[1]; // path relative to the project folder
    if (/^working\//.test(rest))
        return 'draft'; // checked first: a README/file in working/ is still a draft
    if (/^outputs?\//.test(rest))
        return 'published';
    if (rest === 'README.md')
        return 'published'; // project-root README only — anchored by PROJECT_SUBPATH
    if (/^inputs?\//.test(rest))
        return 'reference';
    return undefined;
}
 ...
 */
export function applyProvenance(results) {
