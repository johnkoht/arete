...
export function registerIntegrationCommands(program) {
    ...
    integrationCmd
        ...
        const entries = root
            ...
            : [];
        if (opts.json) {
            console.log(JSON.stringify({ success: true, workspace: root, integrations: entries }, null, 2));
            return;
        }
        ...
        for (const int of entries) {
            const status = int.configured === 'active'
                ? chalk.green(' [active]')
                : int.configured
...
export async function resolveNotionToken(tokenFromOption, promptFn) {
    if (tokenFromOption !== undefined) {
        return tokenFromOption;
    }
    return promptFn();
}
async function promptForNotionToken() {
...
export async function validateNotionToken(token, deps = {}) {
...
export async function saveNotionApiKey(storage, workspaceRoot, token) {
...
export function getNotionMcpSnippet() {
    const snippet = {
        mcpServers: {
            notion: {
                command: 'npx',
