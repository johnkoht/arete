...
export function registerProjectCommand(program) {
    ...
    projectCmd
        ...
        .description('Write the active-project marker (.claude/active-project.json) for <slug>. Harness state only — does not reindex.')
        .option('--json', 'Output as JSON')
        .action(async (slug, opts) => {
        const services = await createServices(process.cwd());
        const root = await services.workspace.findRoot();
    ...
    projectCmd
        .command('mark-dirty')
        .description('Set the active-project marker dirty bit. No-op when no project is open. Harness state only.')
        .option('--json', 'Output as JSON')
        .action(async (opts) => {
        const services = await createServices(process.cwd());
        const root = await services.workspace.findRoot();
        ...
        if (opts.json) {
            console.log(JSON.stringify({ success: true, hadMarker: existing !== undefined, dirty: true }));
            return;
        }
        if (existing) {
    ...
    projectCmd
        .command('mark-clear')
        .description('Delete the active-project marker. No-op when absent. Harness state only.')
        .option('--json', 'Output as JSON')
        .action(async (opts) => {
        const services = await createServices(process.cwd());
        const root = await services.workspace.findRoot();
        ...
        if (opts.json) {
            console.log(JSON.stringify({ success: true, cleared: existing !== undefined }));
            return;
        }
        if (existing) {
    ...
    projectCmd
        .command('list')
        .description('List active projects (slug, name, area, status, lastTouched) sorted by README mtime descending. Read-only.')
        .option('--json', 'Output as JSON')
        .action(async (opts) => {
        const services = await createServices(process.cwd());
        const root = await services.workspace.findRoot();
        ...
        projects.sort((a, b) => {
            if (a.lastTouched === b.lastTouched)
                return 0;
            if (a.lastTouched === null)
                return 1;
            if (b.lastTouched === null)
                return -1;
        ...
        for (const p of projects) {
            ...
            console.log(`  ${chalk.cyan(p.slug.padEnd(36))} ${chalk.dim(touched)}${meta ? chalk.dim(`  (${meta})`) : ''}`);
        }
    });
    // ---------------------------------------------------------------------
    ...
    projectCmd
        .command('open <name>')
        .description('READ-ONLY: resolve a project by name, print its brief + what changed since the README was last touched. Never writes.')
        .option('--json', 'Output as JSON')
        ...
        if (isTie) {
            ...
            if (opts.json) {
                console.log(JSON.stringify({ success: true, disambiguation: true, candidates: list }, null, 2));
            }
            else {
                info(`'${name}' is ambiguous — which project did you mean?`);
                for (const c of list) {
                    console.log(`  ${chalk.cyan(c.slug.padEnd(36))} ${chalk.dim(`score ${c.score}`)}${c.status === 'archived' ? chalk.yellow('  [archived]') : ''}`);
                }
        ...
        if (top.metadata?.status === 'archived') {
            if (opts.json) {
                console.log(JSON.stringify({ success: true, slug: top.slug, archived: true, path: top.path }));
            }
            else {
