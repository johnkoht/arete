...
function htmlToBasicMarkdown(html) {
    ...
    // Convert some tags
    text = text.replace(/<h([1-6])[^>]*>(.*?)<\/h\1>/gi, (_, level, content) => '#'.repeat(Number(level)) + ' ' + content.trim());
...
export async function runInboxAdd(opts, deps = getDefaultDeps()) {
    ...
    if (binaryFileName && opts.file) {
        ...
        try {
            ...
            const errorMsg = `Failed to copy file: ${err.message}`;
            if (opts.json) {
                console.log(JSON.stringify({ success: false, error: errorMsg }));
            }
            else {
