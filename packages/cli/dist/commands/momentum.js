...
import { header } from '../formatters.js';
// ---------------------------------------------------------------------------
// Core implementation (injectable for tests)
// ---------------------------------------------------------------------------
export async function runMomentum(opts, deps = {}) {
    const { computeCommitmentMomentumFn = computeCommitmentMomentum, computeRelationshipMomentumFn = computeRelationshipMomentum, } = deps;
    ...
    const [openCommitments, relationshipMomentum] = await Promise.all([
        services.commitments.listOpen(opts.person ? { personSlugs: [opts.person] } : undefined),
    ...
    if (hot.length === 0 && staleC.length === 0 && critical.length === 0) {
        ...
        if (hot.length > 0) {
            ...
            for (const item of hot) {
                const dirStr = item.commitment.direction === 'i_owe_them'
    ...
    if (active.length === 0 && cooling.length === 0 && staleR.length === 0) {
        ...
        if (active.length > 0) {
            ...
            for (const r of active) {
                console.log(`    ${chalk.bold(r.personName)} ${chalk.dim('·')} last met ${r.lastMeetingDate} ${chalk.dim('·')} ${r.meetingCount} meeting${r.meetingCount === 1 ? '' : 's'}`);
            }
        }
        if (cooling.length > 0) {
            console.log('');
            ...
            for (const r of cooling) {
                console.log(`    ${chalk.bold(r.personName)} ${chalk.dim('·')} last met ${r.lastMeetingDate} ${chalk.dim('·')} ${r.daysSinceMeeting}d ago`);
            }
        }
        if (staleR.length > 0) {
            console.log('');
    ...
    if (critical.length > 0) {
        console.log(chalk.red(`  ⚡ ${critical.length} critical commitment${critical.length === 1 ? '' : 's'} need attention.`));
    }
    if (staleR.length > 0) {
        console.log(chalk.yellow(`  ⚡ ${staleR.length} relationship${staleR.length === 1 ? '' : 's'} drifting.`));
    }
...
// ---------------------------------------------------------------------------
export function registerMomentumCommand(program, deps = {}) {
    program
        .command('momentum')
