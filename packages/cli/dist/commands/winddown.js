...
export function registerWinddownCommand(program) {
    ...
    winddownCmd
        ...
        .description('Mark staged items as confident-keep on a meeting (pre-checks `[x]` in the ' +
        ...
        .action(async (meeting, itemIds, opts) => {
        const services = await createServices(process.cwd());
        const root = await services.workspace.findRoot();
        if (!root) {
            if (opts.json)
        ...
        if (opts.remove)
            success(`Un-elevated ${itemIds.length} item(s) on ${slug}: ${itemIds.join(', ')}`);
        else
            success(`Elevated ${itemIds.length} item(s) on ${slug}: ${itemIds.join(', ')}`);
    });
}
/**
