 ...
 */
import { createServices, loadConfig, getAdapterFromConfig, detectCrossPersonPatterns, } from '@arete/core';
 ...
 */
function countMdFilesInSubdirs(dir) {
    if (!existsSync(dir))
        return 0;
    try {
        let total = 0;
 ...
 */
function countMeetingsByStatus(meetingsDir) {
 ...
 */
function countActiveProjects(projectsDir) {
...
export function registerStatusCommand(program) {
    program
        ...
        const integrationsConfigsDir = join(root, adapter.integrationsDir(), 'configs');
        ...
        console.log(`  ${chalk.dim('🗂  People:')}  ${chalk.bold(String(totalPeopleCount))} ${chalk.dim(`(${internalPeopleCount} internal, ${customerPeopleCount} customers, ${userPeopleCount} users)`)}`);
        const meetingBadge = meetings.unprocessed > 0
            ? chalk.yellow(` (${meetings.unprocessed} unprocessed)`)
            : '';
        console.log(`  ${chalk.dim('📅 Meetings:')} ${chalk.bold(String(meetings.total))} total${meetingBadge}`);
        const commitmentBadge = overdueCount > 0
        ...
        if (integrations.length === 0) {
            ...
            for (const int of integrations) {
                const statusColor = int.status === 'active'
                    ? chalk.green
                    : int.status === 'error'
                        ? chalk.red
                        : chalk.dim;
