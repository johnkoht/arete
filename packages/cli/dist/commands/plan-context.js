...
export function registerPlanContextCommand(program) {
    program
        .command('plan-context')
        .description('Aggregate active-project + topic + goal context for planning. ' +
        ...
        .option('--project <slug>', 'Single-project scope — current state for one project (current-state source per AGENTS.md)')
        ...
        const mode = opts.project ? 'project' : opts.day ? 'day' : 'week';
        const paths = services.workspace.getPaths(root);
        const bundle = await services.intelligence.assemblePlanContext(mode, paths, opts.project ? { projectSlug: opts.project } : {});
        if (opts.json) {
        ...
        for (const p of bundle.projects) {
            ...
            listItem(`  ${p.slug}`, `${p.status ?? 'unknown'} — ${docs} doc(s), ${p.openQuestions.length} open Q${conf}`);
        }
