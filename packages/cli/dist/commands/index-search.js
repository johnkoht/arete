 ...
 */
export function parseVectorCount(statusOutput) {
...
export function registerIndexSearchCommand(program) {
    program
        .command('index')
        .description('Re-index the search collection. For full workspace update (rules, skills, assets), use `arete update`.')
        ...
        if (Object.keys(result.collections).length > 0) {
            await services.workspace.updateManifestField(root, 'qmd_collections', result.collections);
