 ...
 */
export function aggregateCostReport(events, windowDays, grouping, now = new Date()) {
    ...
    const rows = Array.from(buckets.values()).sort((a, b) => grouping === 'day' ? a.group.localeCompare(b.group) : b.costUsd - a.costUsd);
    return {
        windowDays,
...
export async function runCostReport(opts, deps = {}) {
    ...
    const grouping = opts.by === 'skill' ? 'skill' : 'day';
    ...
    for (const row of summary.rows) {
        ...
        console.log(`  ${row.group.padEnd(20)} ${cost.padStart(12)} ${String(row.events).padStart(8)}`);
    }
    console.log(chalk.dim(`  ${'─'.repeat(20)} ${'─'.repeat(12)} ${'─'.repeat(8)}`));
...
async function defaultReadLog(storage, memoryDir) {
    return storage.read(join(memoryDir, 'log.md'));
}
// ---------------------------------------------------------------------------
// Command registration
// ---------------------------------------------------------------------------
export function registerCostCommand(program, deps = {}) {
