 ...
 */
import { createServices, loadConfig, loadReconciliationContext, loadRecentMeetingBatch, nominateCandidates, ledgerEntriesFromBatch, } from '@arete/core';
import { join } from 'path';
import { error, info, success } from '../formatters.js';
export function registerReconcileCommands(program) {
    ...
    reconcileCmd
        ...
        if (days > 0) {
            ...
            const ledgerMeetingPaths = new Set(ledgerEntries.filter((e) => e.kind === 'extraction').map((e) => e.source_ref));
        ...
        info(`duplicates: ${result.stats.duplicatePairs} · uncertain-band: ${result.stats.uncertainBandPairs} · ` +
