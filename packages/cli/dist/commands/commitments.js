...
import { createServices, loadConfig, refreshQmdIndex, buildPersonDirectory, migrateCommitmentsToV2, formatMigrationDiff, parseCommitmentsFile, serializeCommitmentsFile, resolveArchivedProjectReadme, } from '@arete/core';
...
export function registerCommitmentsCommand(program) {
    ...
    commitmentsCmd
        .command('claim <id>')
        .description('Claim a commitment for a project (stamps projectSlug; the project brief\'s Open-work section moves it from unclaimed-area to project-claimed). Use --clear to release.')
        ...
        if (opts.clear) {
            ...
            success(`Claimed ${updated.id.slice(0, 8)} for project ${opts.project} — "${updated.text}"`);
        }
    });
    // ---------------------------------------------------------------------------
    ...
    commitmentsCmd
        ...
        if (raw !== null && raw.trim() !== '') {
            ...
            try {
                ...
                parseError = `commitments.json exists but is malformed (${err instanceof Error ? err.message : String(err)}); refusing to migrate.`;
            }
            if (parseError !== null) {
                if (opts.json)
        ...
        try {
            await services.commitments.withLock(async () => {
                ...
                if (lockedResult.summary.ambiguous > 0) {
                    throw new Error(`commitments.json changed under the lock (${lockedResult.summary.ambiguous} ambiguous row(s) now present); re-run --dry-run before --apply.`);
                }
            ...
            const msg = `Failed to apply migration under lock: ${err instanceof Error ? err.message : String(err)}. Pre-migration snapshot at ${snapshotPath} is the recovery anchor (if written); run \`arete commitments restore --from ${snapshotPath}\` to roll back.`;
            if (opts.json)
                console.log(JSON.stringify({ success: false, error: msg }));
            else
                error(msg);
