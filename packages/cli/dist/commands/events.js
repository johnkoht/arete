 ...
 */
import { createServices, evaluateSlackThread, parseApprovedSection, } from '@arete/core';
 ...
 */
export async function runDeferralDisagreementLog(opts, deps = {}) {
...
function workspaceRelativePath(workspaceRoot, p) {
    if (!isAbsolute(p))
        return p;
 ...
 */
export async function runBackfillItemFates(opts, deps = {}) {
    ...
    if (existsSync(meetingsDir)) {
        ...
        for (const meetingPath of meetings) {
            ...
            const importance = frontmatter.importance ?? null;
            const importanceTyped = importance === 'light' || importance === 'normal' ||
    ...
    success(`Backfill complete: scanned=${scanned} alreadyRecorded=${alreadyRecorded} emitted=${emitted}`);
    return summary;
}
/**
 ...
 */
export function parseSinceDate(raw, now) {
    const v = raw.trim();
    ...
    if (relMatch) {
        ...
        const out = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
 ...
 */
function listMeetingFilesSince(meetingsDir, sinceDate) {
    const out = [];
    walk(meetingsDir);
    return out;
    function walk(dir) {
 ...
 */
function readExistingFateKeys(memoryDir) {
    const keys = new Set();
...
function fateDedupKey(source, kind, text, fate) {
...
export function registerEventsCommand(program, deps = {}) {
    ...
    log
        ...
        .description('Log a deferral_disagreement item-fate (chef detected user pull-back from sidecar)')
    ...
    backfill
        .command('item-fates')
        .description('Scan approved meeting bodies in window; emit fate=approved events for items not yet in item-fates.jsonl. Idempotent.')
