    ...
    areasCmd
        ...
        if (report.dangling.length > 0) {
            ...
            for (const ref of report.dangling) {
                console.log(`    ${chalk.red(ref.value)} ${chalk.dim(`(${ref.count} file${ref.count === 1 ? '' : 's'})`)}`);
        ...
        if (report.duplicateAliases.length > 0) {
            ...
            for (const dup of report.duplicateAliases) {
                console.log(`    ${chalk.red(dup.alias)} ${chalk.dim('claimed by: ' + dup.areas.join(', '))}`);
            }
        }
        if (report.shadowingAliases.length > 0) {
            ...
            for (const shadow of report.shadowingAliases) {
                console.log(`    ${chalk.red(shadow.alias)} ${chalk.dim(`on "${shadow.declaredBy}" shadows area "${shadow.shadows}"`)}`);
            }
        }
