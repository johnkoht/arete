...
describe('search --answer (removed in Phase 7b)', () => {
  it('rejects --answer flag at the parser (unknown option)', () => {
    const tmpDir = createTmpDir('arete-test-search-answer-removed');
    try {
      setupWorkspace(tmpDir, { all: 'test-all' });
      const { code, stderr, stdout } = runCliRaw(
        ['search', 'test query', '--answer'],
        { cwd: tmpDir },
      );

      // Commander rejects unknown options with non-zero exit and an error in stderr.
      assert.notEqual(code, 0, 'CLI should not exit 0 when --answer is supplied');
      const combined = `${stderr}\n${stdout}`;
      assert.ok(
        /unknown option/i.test(combined) && combined.includes('--answer'),
        `Expected "unknown option --answer" in CLI output, got: ${combined}`,
      );
    } finally {
