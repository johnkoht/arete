...
import { listItem } from '../formatters.js';

/**
 * Format a relevance tier as a colored badge for CLI output.
 * HIGH = green, NORMAL = yellow, LOW = red.
 */
export function formatTierBadge(tier: ReconciledItem['relevanceTier']): string {
  switch (tier) {
 ...
 */
export function getReconciledItemText(item: ReconciledItem): string {
  if (typeof item.original === 'string') return item.original;
 ...
 */
export function displayReconciliationDetails(
 ...
 */
export function displayReconciledCompletedItems(
  reconciled: Array<{ id: string; matchedText: string }>,
): void {
  console.log('');
