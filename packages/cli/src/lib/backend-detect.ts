 ...
 */
export async function detectRunningBackend(
  workspaceRoot: string,
  options: BackendDetectOptions = {},
...
function readPidFile(workspaceRoot: string): number | null {
 ...
 */
export function formatBackendWarning(result: BackendDetectResult): string {
  if (!result.running) return '';
  const where =
    result.source === 'pid' ? `pid ${result.pid}` :
