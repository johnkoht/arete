...
export function registerMeetingCommands(program: Command): void {
  ...
  meetingCmd
    ...
    .description(
      'Write `area:` + `area_set_by:` into a meeting\'s frontmatter (body preserved). Area slug must match a file in areas/.',
    )
    ...
    .action(
      async (
        file: string,
        areaSlug: string,
        opts: { setBy?: string; json?: boolean },
      ) => {
        const services = await createServices(process.cwd());
        const root = await services.workspace.findRoot();
        if (!root) {
        ...
        if (setBy !== 'approval' && setBy !== 'manual') {
          if (opts.json) {
            console.log(
              JSON.stringify({ success: false, error: '--set-by must be approval or manual' }),
            );
          } else {
        ...
        if (!(await services.storage.exists(meetingPath))) {
          if (opts.json) {
            console.log(
              JSON.stringify({ success: false, error: `Meeting not found: ${meetingPath}` }),
            );
          } else {
            error(`Meeting not found: ${meetingPath}`);
          }
    ...
    .action(async (file: string, opts: {
      ...
      try {
        ...
        if (config.reconcile_shadow === true && !opts.dryRun) {
          let failureReason: 'call_error' | 'parse_error' | 'truncation' | 'unknown' = 'call_error';
      ...
      let dedupResult: WireExtractDedupResult | undefined;
      ...
      if (shouldStage) {
        ...
        try {
          const extractedItemsForDedup = adaptFilteredItemsForDedup(
            processed.filteredItems.map((fi) => ({
              id: fi.id,
              text: fi.text,
              type: fi.type,
          ...
          if (extractedItemsForDedup.length > 0) {
            ...
            const dedupCallConcurrent = async (
            ...
            dedupResult = await wireExtractDedup(
              { storage: services.storage, commitments: services.commitments },
              {
                workspaceRoot: root,
                meetingsDir,
        ...
        if (dedupResult && dedupResult.decisions.length > 0) {
          stagedSections = decorateStagedSectionsWithDupeBadges(
            stagedSections,
            dedupResult.decisions,
          );
        }
      ...
      const crossMeetingDedup = dedupResult
        ? {
            ...
            definiteDupes: dedupResult.decisions.filter(
              (d) => d.outcome.kind === 'definite-dupe',
            ).length,
            possiblyMergeable: dedupResult.decisions.filter(
              (d) => d.outcome.kind === 'possibly-mergeable',
            ).length,
            newCanonical: dedupResult.decisions.filter(
              (d) => d.outcome.kind === 'new-canonical',
            ).length,
  ...
  meetingCmd
    ...
    .action(async (slug: string, opts: {
      ...
      let topicIntegrationError: { kind: 'seed-lock-held' | 'error'; message: string } | undefined;
      if (!opts.skipTopics && services.ai.isConfigured() && process.env.ARETE_NO_LLM !== '1') {
        try {
      ...
      const legacyApproved = finalFm['approved_items'] as {
        actionItems?: string[];
        decisions?: string[];
        learnings?: string[];
      } | undefined;
