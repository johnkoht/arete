...
export function registerAreasCommands(program: Command): void {
  ...
  areasCmd
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      if (opts.json) {
        console.log(
          JSON.stringify({ success: true, areas, count: areas.length }, null, 2),
        );
        return;
      }
      ...
      console.log('');
      console.log(
        chalk.dim('  Slug                       Name                              Status     Meetings  Epics'),
      );
  ...
  areasCmd
    ...
    .option('--json', 'Output as JSON')
    .action(
      async (opts: { active?: boolean; slug?: string; json?: boolean }) => {
        const services = await createServices(process.cwd());
        const root = await services.workspace.findRoot();
        if (!root) {
          if (opts.json) {
