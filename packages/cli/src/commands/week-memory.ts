...
const WEEK_MEMORY_TYPES: WeekMemoryType[] = [
  'framing-override',
  'deprioritization',
  'week-constraint',
];
...
function entryToJson(e: WeekMemoryEntry): Record<string, unknown> {
  return {
    id: e.id,
...
export function registerWeekMemoryCommand(program: Command): void {
  ...
  weekMemoryCmd
    ...
    .requiredOption(
      '--type <type>',
      `Entry type: ${WEEK_MEMORY_TYPES.join(', ')}`,
    )
    ...
    .action(
      async (opts: {
        ...
        if (!WEEK_MEMORY_TYPES.includes(opts.type as WeekMemoryType)) {
        ...
        try {
          result = await addWeekMemoryEntry(services.storage, root, {
            type: opts.type as WeekMemoryType,
  ...
  weekMemoryCmd
    ...
    .action(async (opts: { active?: boolean; json?: boolean }) => {
      ...
      for (const e of entries) {
        ...
        const statusTag =
          e.status === 'resolved' ? chalk.dim('(resolved)') : '';
