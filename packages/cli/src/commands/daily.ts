...
export type DailyBrief = {
  ...
  overdueCommitments: Array<{ commitment: Commitment; daysOverdue: number }>;
 ...
 */
function defaultPullCalendar(_workspaceRoot: string): CalendarEvent[] {
  try {
 ...
 */
async function gatherRecentDecisions(
  workspaceRoot: string,
  storage: StorageAdapter,
 ...
 */
function filterOverdueCommitments(
  commitments: Commitment[],
): Array<{ commitment: Commitment; daysOverdue: number }> {
  const now = new Date();
...
export async function runDaily(
  ...
  const {
    pullCalendarFn = defaultPullCalendar,
    detectPatternsFn = detectCrossPersonPatterns,
  } = deps;
  ...
  const [meetings, openCommitments, activeProjects, recentDecisions, patterns] =
    await Promise.all([
  ...
  if (brief.overdueCommitments.length === 0) {
    ...
    if (brief.overdueCommitments.length > 5) {
      console.log(
        chalk.dim(
          `  ... and ${brief.overdueCommitments.length - 5} more. Run \`arete commitments list\` to see all.`,
        ),
      );
    }
  }
  ...
  if (brief.patterns.length === 0) {
    console.log(
      chalk.dim('  No cross-person patterns detected in the last 30 days'),
    );
  } else {
    for (const p of brief.patterns) {
      ...
      console.log(
        chalk.dim(
          `    ${p.mentions} meetings · ${peopleStr}${morePeople} · last ${p.lastSeen}`,
        ),
      );
    }
  }
  ...
  // Footer hints
  console.log(
    chalk.dim('  Run `arete momentum` for commitment and relationship momentum.'),
  );
  console.log(
    chalk.dim('  Run `arete commitments list` to manage open commitments.'),
  );
  console.log('');
}
...
export function registerDailyCommand(program: Command, deps: DailyCommandDeps = {}): void {
  program
    .command('daily')
