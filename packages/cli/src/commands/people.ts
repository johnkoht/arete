...
import { promises as fsp, existsSync, readFileSync, writeFileSync, readdirSync } from 'node:fs';
      ...
      if (useLLM) {
            ...
            for (const person of targetPeople) {
              ...
              for (const meetingFile of meetingFiles) {
                ...
                try {
                  ...
                  if (content.toLowerCase().includes(nameLower)) stanceCallCount++;
                } catch {
                  // skip unreadable
                }
              }
        ...
        if (estimatedCost > COST_CEILING_USD) {
          ...
          const ok = await confirmInteractive(
            `Estimated $${estimatedCost.toFixed(2)} (${stanceCallCount} stance calls). Above $${COST_CEILING_USD.toFixed(2)} ceiling — proceed?`,
          );
          if (!ok) {
          ...
          info(
            `Will extract stances for ~${stanceCallCount} person×meeting pairs — estimated cost ~$${estimatedCost.toFixed(2)}.`,
          );
 ...
 */
async function writePreRefreshSnapshot(
  paths: { people: string; root: string },
  snapshotPath: string,
  personSlugFilter?: string,
): Promise<void> {
  const START = '<!-- AUTO_PERSON_MEMORY:START -->';
  const END = '<!-- AUTO_PERSON_MEMORY:END -->';
  const blocks: Array<{ path: string; relativePath: string; block: string | null }> = [];
  for (const cat of PEOPLE_CATEGORIES) {
    ...
    for (const f of files) {
      ...
      if (personSlugFilter && slug !== personSlugFilter) continue;
      ...
      try {
        ...
        const block =
          start >= 0 && end > start ? content.slice(start, end + END.length) : null;
        blocks.push({
          path: filePath,
