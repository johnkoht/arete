...
import {
  ...
  formatExplainReport,
  type BackgroundDedupScope,
  type RunBackgroundDedupInputs,
...
const VALID_SCOPES: BackgroundDedupScope[] = [
  'commitments',
  'decisions',
  'learnings',
  'topics',
...
export function registerDedupCommand(program: Command): void {
  program
    .command('dedup')
    .description(
      'Background dedup hygiene pass. Reuses the reactive hybrid pipeline retroactively across an --since window. Default is --dry-run (writes diff report only); --apply mutates commitments.json under lock.',
    )
    .option(
      '--scope <scope>',
      `Scope: ${VALID_SCOPES.join(' | ')} (required unless --explain is used)`,
    )
    .option(
      '--explain <commitment-id>',
      'Print dedup provenance for a single commitment (8-char prefix or full hash): canonical text, stakeholders, source meetings with merge events, textVariants, and dedup-decisions log entries. Read-only.',
    )
    .option(
      '--dry-run',
      'Default — produce diff report without writing any data',
    )
    .option(
      '--apply',
      'Write the merge result (commitments scope only in v2). Acquires the commitments lock before reading + writing.',
    )
    .option(
      '--since <YYYY-MM-DD>',
      'Limit scope to entries from this date forward (default: all-time)',
    )
    .option(
      '--llm',
      'Invoke the AIService LLM for ambiguous-pair cross-check (default: deterministic Jaccard-only, surfaces fuzzy pairs as candidates without LLM verdicts)',
    )
    .option(
      '--tier <tier>',
      'LLM tier: fast | standard | frontier (default: fast)',
      'fast',
    )
    .option(
      '--diff-dir <path>',
      'Directory for the diff report (default: dev/work/plans/arete-v2-chef-orchestrator/phase-10-winddown-orchestrator)',
    )
    ...
    .action(
      async (opts: {
        ...
        if (!opts.scope) {
          const msg =
            'Missing required option: --scope (or use --explain <commitment-id>)';
        ...
        // Validate scope.
        if (!VALID_SCOPES.includes(opts.scope as BackgroundDedupScope)) {
          const msg = `Invalid --scope: "${opts.scope}". Must be one of: ${VALID_SCOPES.join(', ')}`;
          if (opts.json) console.log(JSON.stringify({ success: false, error: msg }));
          else error(msg);
          process.exit(1);
        }
        const scope = opts.scope as BackgroundDedupScope;

        // Validate tier.
        const tier = (opts.tier ?? 'fast') as 'fast' | 'standard' | 'frontier';
        ...
        const callConcurrent = opts.llm
          ? (async (
            ...
            ) => services.ai.callConcurrent(prompts))
          : undefined;

        // Build inputs based on scope.
        let result: BackgroundDedupResult;
        try {
        ...
        if (scope !== 'commitments') {
          ...
          if (!opts.json) {
            info(
              `--apply for scope=${scope} is surface-only in v2 (per plan AC10a). Diff written; no data mutated. Manual merge guidance lives in the diff report.`,
            );
          }
          renderSuccess({
            json: opts.json,
        ...
        try {
          await services.commitments.withLock(async () => {
            ...
            const next = applyCommitmentsDedup(
              lockedCommitments,
              lockedResult,
            );
            ...
            for (const payload of collectDupeProvenance(
              lockedCommitments,
              lockedResult,
            )) {
 ...
 */
async function runExplain(idOrPrefix: string, json: boolean): Promise<void> {
  const services = await createServices(process.cwd());
  const root = await services.workspace.findRoot();
  if (!root) {
    const msg = 'Not in an Areté workspace';
  ...
  if (lookup.kind === 'ambiguous') {
    ...
    if (json) {
      console.log(
        JSON.stringify({ success: false, error: msg, matches: ids }),
      );
    } else error(msg);
    process.exit(1);
  }
  ...
  try {
    ...
    if (rawLog !== null) logEntries = parseDedupLog(rawLog);
  } catch {
...
async function loadInputsForScope(
  ...
  opts: {
    since?: string;
    dryRun: boolean;
    callConcurrent?: RunBackgroundDedupInputs['callConcurrent'];
    tier: 'fast' | 'standard' | 'frontier';
  },
): Promise<RunBackgroundDedupInputs> {
  switch (scope) {
    case 'commitments': {
      const commitmentsPath = join(root, '.arete/commitments.json');
    ...
    case 'learnings': {
      ...
      if (raw !== null) {
        ...
        for (const s of parsed) {
          const sec: MemorySectionInput = {
            title: s.title,
            body: s.body,
          };
          if (s.date) sec.date = s.date;
          if (s.source) sec.source = s.source;
          if (s.topics) sec.topics = s.topics;
          sections.push(sec);
        }
      }
      return {
    ...
    case 'topics': {
      ...
      const topics: TopicPageInput[] = pages.map((p) => {
        const body = Object.values(p.sections)
          .filter((s) => typeof s === 'string')
          .join('\n\n');
        const t: TopicPageInput = {
    ...
    default: {
      ...
      throw new Error(`Unknown scope: ${_exhaustive as string}`);
    }
  }
}
...
function renderSuccess(args: {
  ...
  mode: 'dry-run' | 'apply';
  scope: BackgroundDedupScope;
  summary: BackgroundDedupResult['summary'];
  diffPath: string;
  applied: boolean;
  since?: string;
}): void {
  ...
  if (args.applied) {
    success(
      `Applied ${args.scope} dedup: ${args.summary.duplicates} duplicate(s) absorbed across ${args.summary.groups} group(s).`,
    );
  } else {
    success(
      `${args.mode === 'dry-run' ? 'Dry-run' : 'Apply (surface-only)'} complete for scope=${args.scope}: ${args.summary.totalIn} items in scope, ${args.summary.groups} group(s), ${args.summary.uncertain} pair(s) for review.`,
    );
  }
  listItem('Diff report', args.diffPath);
  listItem(
    'Summary',
    `totalIn=${args.summary.totalIn} groups=${args.summary.groups} duplicates=${args.summary.duplicates} uncertain=${args.summary.uncertain}`,
  );
  if (args.mode === 'dry-run') {
    console.log('');
    info(
      `To apply (commitments scope only in v2): rerun with --apply. Memory scopes are surface-only; edit the source files using the diff as guidance.`,
    );
  }
  console.log('');
}
