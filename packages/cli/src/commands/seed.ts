...
type SeedStats = {
  ...
  memory: number;
  context: number;
  testScenarios: boolean;
};

function copyFileIfNeeded(sourcePath: string, destinationPath: string, force: boolean): boolean {
...
async function seedFromFixtures(
  ...
  if (existsSync(meetingsSource)) {
    ...
    for (const file of files) {
      const copied = copyFileIfNeeded(
        join(meetingsSource, file),
        join(meetingsDestination, file),
        force,
      );
      if (copied) {
        stats.meetings += 1;
  ...
  if (existsSync(plansSource)) {
    ...
    if (week) {
      const copied = copyFileIfNeeded(
        join(plansSource, week),
        join(paths.now, 'week.md'),
        force,
      );
      if (copied) {
        stats.plans += 1;
  ...
  if (existsSync(contextSource)) {
    ...
    for (const file of contextFiles) {
      if (file === 'goals-strategy.md') {
        const copied = copyFileIfNeeded(
          join(contextSource, file),
          join(paths.goals, 'strategy.md'),
          force,
        );
        if (copied) {
          stats.context += 1;
  ...
  program
    ...
    .action(
      async (
        ...
        opts: { days?: number; yes?: boolean; force?: boolean; json?: boolean },
      ) => {
        const services = await createServices(process.cwd());
