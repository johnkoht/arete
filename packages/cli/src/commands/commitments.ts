...
export function registerCommitmentsCommand(program: Command): void {
  ...
  commitmentsCmd
    ...
    .description(
      'Propose auto-resolutions from Gmail Sent evidence (Phase 11 — GATED OFF, proposals only)',
    )
    .option('--json', 'Output as JSON')
    .action(async (opts: { json?: boolean }) => {
      ...
      if (!gateOn) {
        const msg =
          'Phase 11 auto-resolve is gated off pending golden-pair validation. ' +
      ...
      const paths = services.workspace.getPaths(root);
      const people = await services.entity.listPeople(paths);
      const slugEmailMap: Record<string, string> = {};
      ...
      if (opts.json) {
        console.log(
          JSON.stringify({ success: true, gated: false, proposals }, null, 2),
        );
        return;
      }

      success(
        `Scanned ${open.length} open commitment(s); ${proposals.length} proposal(s). ` +
