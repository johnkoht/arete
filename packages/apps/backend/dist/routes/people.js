...
async function loadCommitments(workspaceRoot) {
    try {
...
function parseAutoMemoryBlock(content) {
    ...
    if (lastMetMatch)
        lastMeetingDate = lastMetMatch[1] ?? null;
    ...
    if (statusMatch)
        healthStatus = (statusMatch[1] ?? '').trim();
    // Compute health score from meetings frequency (heuristic)
    if (meetingsLast30d > 0) {
        if (meetingsLast30d >= 4)
            healthScore = 90;
        else if (meetingsLast30d >= 2)
            healthScore = 70;
        else
            healthScore = 50;
    }
...
function parseRecentMeetings(content) {
...
function computeTrend(meetingsLast30d, meetingsLast90d) {
    if (meetingsLast30d === 0 && meetingsLast90d === 0)
        return null;
    const avg30 = meetingsLast30d;
    const avg60to90 = Math.max(0, meetingsLast90d - meetingsLast30d) / 2;
    if (avg30 > avg60to90 + 0.5)
        return 'up';
    if (avg30 < avg60to90 - 0.5)
        return 'down';
    return 'flat';
}
async function scanPeopleDir(workspaceRoot) {
    const categories = ['internal', 'customer', 'user'];
    const results = [];
        ...
        try {
            ...
            for (const { slug, category, filePath } of personFiles) {
                try {
                    ...
                    // Commitments count
                    const openCommitments = commitments.filter((c) => c.personSlug === slug && c.status === 'open').length;
                    ...
                    const lastMeetingTitle = recentMeetings.length > 0 ? (recentMeetings[0]?.title ?? null) : null;
