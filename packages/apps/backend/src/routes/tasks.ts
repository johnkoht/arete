...
/**
 * Wire format for tasks sent to the frontend.
 */
interface TaskWire {
  id: string;
  text: string;
...
async function enrichTask(
  ...
  if (task.metadata.from?.type === 'commitment') {
    ...
    const commitment = allCommitments.find(
      (c) => c.id === commitmentId || c.id.startsWith(commitmentId),
    );
    if (commitment) {
      const daysOpen = Math.floor(
        (Date.now() - new Date(commitment.date).getTime()) / (1000 * 60 * 60 * 24),
      );
...
export function createTasksRouter(workspaceRoot: string): Hono {
  const app = new Hono();
    ...
    try {
      ...
      if (filterParam === 'today') {
        ...
        filteredTasks = allTasks.filter(
          (t) => t.source.section === '## Anytime' && !t.completed,
        );
      ...
      if (waitingOn) {
        filteredTasks = filteredTasks.filter(
          (t) => t.metadata.from?.type === 'commitment',
        );
      }
      ...
      const enrichedTasks = await Promise.all(
        paginated.map((t) => enrichTask(t, allCommitments, workspaceRoot)),
      );
