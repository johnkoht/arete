 ...
 */
export function extractExcerpt(content: string, queryTokens: string[]): string {
 ...
 */
function countOccurrences(content: string, queryTokens: string[]): number {
 ...
 */
function matches(content: string, queryTokens: string[]): boolean {
  const lower = content.toLowerCase();
...
async function scanMeetings(
  ...
  for (const file of files) {
    ...
    if (!content || !matches(content, queryTokens)) continue;
...
async function scanMemory(
  ...
  for (const file of memoryFiles) {
    ...
    const type: 'decision' | 'learning' = file === 'decisions.md' ? 'decision' : 'learning';
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (!matches(section, queryTokens)) continue;
...
export function createSearchRouter(workspaceRoot: string): Hono {
  const app = new Hono();
