...
// ---------------------------------------------------------------------------
// PullResult type contract — reconciliation field
// ---------------------------------------------------------------------------

describe('PullResult reconciliation field', () => {
  it('PullResult type accepts reconciliation data', () => {
    // Type-level verification that PullResult carries reconciliation.
    // If this compiles, the field is correctly defined.
    const result: import('../../src/models/integrations.js').PullResult = {
      integration: 'fathom',
      itemsProcessed: 2,
      itemsCreated: 2,
      itemsUpdated: 0,
        ...
        items: [
          {
            ...
            text: 'Follow up on design review',
            source: { meetingSlug: 'meeting-2025-01-01', extractionIndex: 0 },
            status: 'new' as const,
            relevanceScore: 0.8,
          },
        ],
    ...
    assert.ok(result.reconciliation, 'reconciliation field should be present');
    assert.equal(result.reconciliation!.stats.duplicatesRemoved, 1);
    assert.equal(result.reconciliation!.items.length, 1);
  });

  it('PullResult reconciliation is optional', () => {
    const result: import('../../src/models/integrations.js').PullResult = {
      integration: 'fathom',
      itemsProcessed: 0,
      itemsCreated: 0,
    ...
    assert.equal(result.reconciliation, undefined, 'reconciliation should be undefined when not set');
  });
});
