...
describe('AreaMemoryService', () => {
  ...
  describe('refreshAllAreaMemory', () => {
    ...
    it('mechanical-only result post-Phase-7b: no synthesis field, no _synthesis.md written', async () => {
      const areas = [
        makeAreaContext({ slug: 'area-1', name: 'Area One' }),
        makeAreaContext({ slug: 'area-2', name: 'Area Two' }),
      ];
      ...
      assert.equal(result.updated, 2, 'mechanical refresh still writes area files');
      assert.equal(result.scannedAreas, 2);
      assert.equal(result.skipped, 0);
      // Post-7b: the synthesis field no longer exists in RefreshAreaMemoryResult.
      assert.equal((result as unknown as { synthesis?: unknown }).synthesis, undefined);

      // _synthesis.md must never be produced.
      const synthPath = join(WORKSPACE_ROOT, '.arete/memory/areas/_synthesis.md');
      assert.equal(storage.store.has(synthPath), false, '_synthesis.md should not be written');
    });
  });
