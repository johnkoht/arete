...
import { loadKrispCredentials, saveKrispCredentials, } from './config.js';
...
export class KrispMcpClient {
     ...
     */
    async configure(storage, workspaceRoot) {
        ...
        try {
            ...
            authorizationCode = await new Promise((resolve, reject) => {
    ...
    async refreshTokens(creds) {
        ...
        if (res.status === 401) {
            throw new Error('Both tokens expired — run `arete integration configure krisp` to reconnect');
        }
        if (!res.ok) {
    ...
    async callTool(name, args) {
        ...
        if (!creds) {
            throw new Error('Krisp credentials not found — run `arete integration configure krisp`');
        }
        ...
        if (res.status === 401) {
            throw new Error('Krisp session expired — run `arete integration configure krisp`');
        }
        if (res.status === 403) {
