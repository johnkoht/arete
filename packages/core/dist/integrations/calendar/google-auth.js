 ...
 */
export async function loadGoogleCredentials(storage, workspaceRoot) {
 ...
 */
export async function saveGoogleCredentials(storage, workspaceRoot, credentials) {
    const { join } = await import('path');
    const credPath = join(workspaceRoot, '.credentials', 'credentials.yaml');
    let existing = {};
    const exists = await storage.exists(credPath);
...
// Error formatting
// ---------------------------------------------------------------------------
function isOAuthError(value, expected) {
...
export async function refreshToken(credentials) {
    ...
    if (!res.ok) {
        ...
        if (isOAuthError(errorBody, 'invalid_client')) {
            throw new Error('Google Calendar client configuration is invalid. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET environment variables and try again.');
        }
        if (res.status === 429) {
        ...
        throw new Error(`Google Calendar token refresh failed (HTTP ${res.status}) — run: ${CONFIGURE_COMMAND}`);
    }
...
function openBrowser(url) {
    return new Promise((resolve, reject) => {
        ...
        exec(command, (err) => {
            if (err)
                reject(new Error('Unable to open your browser automatically.'));
            else
                resolve();
        });
    });
 ...
 */
export async function authenticate(storage, workspaceRoot) {
    const { clientId, clientSecret } = getClientCredentials();
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Could not open your browser automatically. Open this URL manually to continue:\n${authUrl.toString()}`);
        }
            ...
            server.on('request', (req, res) => {
                try {
                    ...
                    if (error) {
                        ...
                        reject(new Error(`Google Calendar authorization was cancelled or denied (${error}) — run: ${CONFIGURE_COMMAND}`));
                        return;
                    }
                    ...
                    if (!code) {
                        ...
                        reject(new Error(`No authorization code received from Google — run: ${CONFIGURE_COMMAND}`));
                        return;
                    }
        ...
        if (!tokenRes.ok) {
            ...
            if (isOAuthError(tokenErrorBody, 'invalid_client')) {
                throw new Error('Google Calendar client configuration is invalid. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET environment variables and retry.');
            }
        ...
        if (typeof tokens.access_token !== 'string' ||
            ...
            throw new Error('Google Calendar did not return complete OAuth tokens. Re-run setup and make sure you grant requested access.');
        }
        const credentials = {
            access_token: tokens.access_token,
