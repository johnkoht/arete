 ...
 */
export function gmailSentCachePath(workspaceRoot, dateYYYYMMDD) {
 ...
 */
export function buildRecipientIndex(threads) {
    const index = {};
    for (const t of threads) {
        ...
        for (const raw of allRecipients) {
            ...
            if (!index[norm])
                index[norm] = [];
            if (!index[norm].includes(t.id))
                index[norm].push(t.id);
        }
    }
    return index;
}
 ...
 */
export async function writeGmailSentCache(storage, workspaceRoot, threads, opts) {
 ...
 * the cache-bust.
 */
export async function readGmailSentCache(storage, workspaceRoot, dateYYYYMMDD) {
    ...
    try {
        ...
        const msg = `[gmail-sent-cache] Unparseable JSON at ${path}: ${err.message}. Will refetch.`;
        console.warn(msg);
    ...
    return {
        ...
        cache: {
            ...
            threads: obj.threads,
            recipientIndex: obj.recipientIndex,
        },
    };
}
/**
 ...
 */
export async function deleteGmailSentCache(storage, workspaceRoot, dateYYYYMMDD) {
    const path = gmailSentCachePath(workspaceRoot, dateYYYYMMDD);
    try {
...
//# sourceMappingURL=gmail-sent-cache.js.map