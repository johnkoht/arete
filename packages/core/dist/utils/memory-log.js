...
export function parseEvent(line) {
    ...
    if (kvPart !== undefined && kvPart.length > 0) {
        ...
        for (const pair of kvPart.split(' ')) {
            ...
            if (eq < 0)
                continue; // malformed token; skip conservatively
 ...
 */
export function appendEvent(existingContent, event) {
 ...
 */
export function appendEvents(existingContent, events) {
    if (events.length === 0)
 ...
 */
export function encodeValue(raw) {
 ...
 * injection in tests.
 */
export function nowIsoSeconds(ref = new Date()) {
