 ...
 */
export async function listProjectsForBackfill(storage, paths) {
    const activeDir = join(paths.projects, 'active');
    if (!(await storage.exists(activeDir)))
        return [];
    const out = [];
 ...
 */
export async function applyAreaToProjectReadme(storage, readmePath, areaSlug, setBy = 'backfill') {
    const content = await storage.read(readmePath);
    if (!content)
        throw new Error(`Project README not found: ${readmePath}`);
    const { fm, body } = parseReadme(content);
    fm['area'] = areaSlug;
 ...
 */
export async function resetBackfilledProjectAreas(storage, paths) {
    const reset = [];
    for (const candidate of await listProjectsForBackfill(storage, paths)) {
        ...
        const fmText = stringifyYaml(fm).trimEnd();
        await storage.write(candidate.readmePath, `---\n${fmText}\n---\n\n${body.replace(/^\n+/, '')}`);
        reset.push(candidate.slug);
    }
    return { reset };
}
//# sourceMappingURL=project-area.js.map