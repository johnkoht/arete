...
export function parseMeetingExtractionResponse(response, limits = CATEGORY_LIMITS, ownerSlug) {
    ...
    const { kept: dedupedDecisions, filtered: dedupedDecisionsFiltered } = deduplicateItems(decisions.map((d, i) => ({ text: d, conf: decisionConfidences[i] })));
    ...
    const { kept: dedupedLearnings, filtered: dedupedLearningsFiltered } = deduplicateItems(learnings.map((l, i) => ({ text: l, conf: learningConfidences[i] })));
