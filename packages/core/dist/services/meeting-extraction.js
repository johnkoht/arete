...
export class EmptyExtractionError extends Error {
    ...
    transcriptChars;
    constructor(transcriptChars) {
        super(`single_pass extraction returned EMPTY intelligence for a ${transcriptChars}-char ` +
 ...
 */
export function isEmptyIntelligence(intel) {
    return (intel.actionItems.length === 0 &&
 ...
 */
const VALID_DIRECTIONS_SINGLE_PASS = new Set(['i_owe_them', 'they_owe_me', 'none']);
const VALID_IMPORTANCE = new Set(['blocker', 'high', 'normal']);
 ...
 */
export function isTrivialDecision(text) {
    if (DECISION_VERBS.test(text))
        return null; // Contains a real decision verb — keep it
 ...
 */
export function isTrivialLearning(text) {
 ...
 */
function isGarbageDecisionOrLearning(text) {
    const lower = text.toLowerCase().trim();
    for (const prefix of GARBAGE_PREFIXES) {
        if (lower.startsWith(prefix)) {
            return `starts with "${prefix}"`;
 ...
 */
export function buildKnownItemsSection(context, priorItems) {
    const actionItems = [];
    const decisions = [];
    const learnings = [];
    if (priorItems && priorItems.length > 0) {
 ...
 */
export function buildSinglePassExtractionPrompt(transcript, opts) {
 ...
 */
export function parseMeetingExtractionResponse(response, limits = CATEGORY_LIMITS, ownerSlug, opts) {
    ...
    const previewOf = (t) => t.slice(0, 50) + (t.length > 50 ? '...' : '');
    // Parse summary
    ...
    if (Array.isArray(raw.topics)) {
        for (const t of raw.topics) {
            ...
            if (!TOPIC_SLUG_REGEX.test(slug))
                continue;
            if (TOPIC_BANNED.has(slug))
                continue;
    ...
    const { kept: dedupedDecisions, filtered: dedupedDecisionsFiltered } = deduplicateItems(decisions.map((d, i) => ({ text: d, conf: decisionConfidences[i], meta: decisionMeta[i] })));
    ...
    const { kept: dedupedLearnings, filtered: dedupedLearningsFiltered } = deduplicateItems(learnings.map((l, i) => ({ text: l, conf: learningConfidences[i], meta: learningMeta[i] })));
...
export async function extractMeetingIntelligence(transcript, callLLM, options) {
    ...
    if (singlePass) {
        ...
        if (transcript.trim().length >= EMPTY_EXTRACTION_MIN_TRANSCRIPT_CHARS &&
...
export function formatStagedSections(result) {
    ...
    if (mirrorPairWarnings.length > 0) {
        ...
        lines.push('_The extractor dropped these items as mirror-pair duplicates of another action ' +
    ...
    if (mirrorPairEvents.length > 0) {
        ...
        lines.push('_The mirror-pair detector flagged these staged action items as possible ' +
 ...
 */
export function updateMeetingContent(originalContent, stagedSections, extraStagedHeaders) {
