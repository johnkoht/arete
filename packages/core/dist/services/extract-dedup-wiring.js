...
import { runExtractDedup, buildDupeSkipReasonEntries, buildDupeStatusEntries, } from './commitment-dedup-extract.js';
...
import { parseStagedSections, parseStagedItemStatus, parseStagedItemOwner, } from '../integrations/staged-items.js';
// ---------------------------------------------------------------------------
 ...
 * Exported for tests.
 */
export async function loadSameDayStagedItems(storage, meetingsDir, meetingDate, excludeSlug) {
    const out = [];
    ...
    for (const filePath of files) {
        ...
        for (const item of sections.actionItems) {
            ...
            if (rawDirection !== 'i_owe_them' && rawDirection !== 'they_owe_me')
                continue;
            ...
            if (ownerSlug)
                personSlugs.push(ownerSlug.toLowerCase());
            if (counterpartySlug)
                personSlugs.push(counterpartySlug.toLowerCase());
            out.push({
                id: `${slug}::${item.id}`,
                text: item.text,
                direction,
 ...
 * Exported for tests.
 */
export async function resolveMeetingSlugToPath(storage, meetingsDir, slug) {
 ...
 */
export async function wireExtractDedup(services, inputs, callConcurrent, options = {}) {
    ...
    await services.commitments.withLock(async () => {
        ...
        const sameDayStagedItems = await loadSameDayStagedItems(services.storage, inputs.meetingsDir, inputs.meetingDate, inputs.currentMeetingSlug);
        decisions = await runExtractDedup(inputs.extractedItems, {
            existingCommitments,
            sameDayStagedItems,
    ...
    const reverseStampResults = [];
    if (!options.dryRun) {
        const stampedSlugs = new Set(); // de-dupe within this extract
        for (const d of decisions) {
            ...
            // re-run after sync).
            if (canonicalSlug === inputs.currentMeetingSlug)
                continue;
            if (stampedSlugs.has(canonicalSlug))
                continue;
            stampedSlugs.add(canonicalSlug);
            const canonicalPath = await resolveMeetingSlugToPath(services.storage, inputs.meetingsDir, canonicalSlug);
            if (!canonicalPath) {
    ...
    if (!options.dryRun) {
        await appendDedupDecisionLogBatch(inputs.workspaceRoot, decisions, tier);
    }
    return {
        decisions,
        skipReasonPatch,
 ...
 */
export function adaptFilteredItemsForDedup(filteredItems) {
    const out = [];
    for (const fi of filteredItems) {
        if (fi.type !== 'action')
            continue;
        const direction = fi.ownerMeta?.direction;
        if (direction !== 'i_owe_them' && direction !== 'they_owe_me')
            continue;
        const personSlugs = [];
        if (fi.ownerMeta?.ownerSlug)
            personSlugs.push(fi.ownerMeta.ownerSlug.toLowerCase());
        if (fi.ownerMeta?.counterpartySlug)
            personSlugs.push(fi.ownerMeta.counterpartySlug.toLowerCase());
        out.push({
            itemId: fi.id,
            text: fi.text,
            direction,
...
//# sourceMappingURL=extract-dedup-wiring.js.map