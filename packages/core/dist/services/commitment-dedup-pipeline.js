 ...
 */
import { normalizeCommitmentTextV2, computeCommitmentHashV2, } from './commitments-hash-v2.js';
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
 ...
 */
export function tokenizeForJaccard(normalizedText) {
 ...
 * Exported for test introspection.
 */
export function buildPersonSlugSet(text, personSlugs) {
    const set = new Set();
 ...
 */
export function findDedupCandidates(extractedItem, existingCommitments, 
// eslint-disable-next-line @typescript-eslint/no-unused-vars
sameDay = true) {
    // ── 1. Exact text-hash match (AC2) ──────────────────────────────────────
    const newHash = computeCommitmentHashV2(extractedItem.text, extractedItem.direction);
    for (const ex of existingCommitments) {
        if (ex.direction !== extractedItem.direction)
            continue;
 ...
 */
export function buildCrossCheckPrompt(newItem, candidates) {
    const lines = [];
    lines.push('You are deciding whether commitments refer to the same intended action.');
    lines.push('');
    lines.push('Two commitments are SAME only when:');
    ...
    lines.push('');
    lines.push('Different recipients, different artifacts, or different timing = DIFFERENT.');
    ...
    for (let i = 0; i < candidates.length; i += 1) {
        ...
        lines.push(`${i + 1}. (from meeting <${c.meetingSlug}>) ${c.text}`);
    }
    lines.push('');
 ...
 * Exported for test introspection.
 */
export function parseCrossCheckResponse(response, candidates) {
    const decisions = [];
    const seen = new Map();
    for (const rawLine of response.split(/\r?\n/)) {
        ...
        const m = line.match(/^(\d+)[.)\]:]\s*(SAME|DIFFERENT|UNCERTAIN)\b\s*(?:[|\-—]\s*(.*))?$/i);
        if (!m)
            continue;
 ...
 */
export async function runLLMCrossCheck(newItem, candidates, callConcurrent, tier = 'fast') {
 ...
 *   candidates is empty).
 */
export function applyDedupDecisions(newItem, candidates, decisions) {
 ...
 */
export async function runDedupPipeline(extractedItem, existingCommitments, callConcurrent, options = {}) {
    ...
    const decisions = await runLLMCrossCheck(extractedItem, candidates, callConcurrent, tier);
 ...
 */
export function commitmentToDedupInput(c) {
    const personSlugs = [];
    if (Array.isArray(c.stakeholders) && c.stakeholders.length > 0) {
        for (const sh of c.stakeholders) {
            if (sh.role === 'self')
                continue;
            if (sh.slug)
                personSlugs.push(sh.slug.toLowerCase());
        }
    }
...
//# sourceMappingURL=commitment-dedup-pipeline.js.map