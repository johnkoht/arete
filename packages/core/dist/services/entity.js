 ...
 */
export function normalizeStanceTokens(text) {
    return new Set(text
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, ' ')
        .split(/\s+/)
 ...
 * Exported for unit testing.
 */
export function stanceJaccardSimilarity(a, b) {
 ...
 * Exported for unit testing.
 */
export function dedupeStancesByJaccard(stances, threshold = STANCE_JACCARD_DEDUP_THRESHOLD) {
    const kept = [];
    // Cache token sets per kept stance, scoped by direction for fast lookup.
    const tokensByDirection = new Map();
