 ...
 */
function computeHash(text, personSlug, direction) {
 ...
 */
function inferDirectionFromText(text, personSlug, ownerSlug) {
    const textLower = text.toLowerCase();
    const words = textLower.split(/\s+/).slice(0, 4).join(' ');
    // Check if owner appears as actor (start of text)
    const ownerIsActor = words.includes(ownerSlug.replace(/-/g, ' ')) ||
    ...
    const personInText = textLower.includes(personSlug.replace(/-/g, ' ')) ||
        textLower.includes(personSlug.split('-')[0]);
    // Check if person appears as actor
    const personIsActor = words.includes(personSlug.replace(/-/g, ' ')) ||
 ...
 */
function extractActionItemsSection(content) {
 ...
 */
export function parseActionItemsFromMeeting(content, personSlug, ownerSlug, source, meetingArea) {
    ...
    for (const line of lines) {
        ...
        if (NONE_DIRECTION_MARKER.test(line))
            continue;
...
//# sourceMappingURL=meeting-parser.js.map