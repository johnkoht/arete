 ...
 */
export function parseApprovedSection(body, sectionName) {
    ...
    const headerRe = new RegExp(`^##\\s+Approved\\s+${escapedName}[ \\t]*\\r?\\n`, 'mi');
...
function extractIntelligenceFromFrontmatter(frontmatter, body) {
    ...
    for (const item of staged.actionItems) {
        ...
        actionItems.push({
            ...
            direction: (ownerMeta?.direction ?? item.direction ?? 'i_owe_them'),
    ...
    // frontmatter.approved_items.
    const legacyApproved = frontmatter.approved_items;
