 ...
 * `other`.
 */
export function inferTemplateType(title, attendeeCount) {
    const t = title.toLowerCase();
    if (/\b1:1\b|\bone[- ]on[- ]one\b|\bweekly\b|\bcheck[- ]?in\b/.test(t))
        return 'one-on-one';
    if (attendeeCount <= 2)
        return 'one-on-one';
    return 'other';
 ...
 */
export function deriveRecurringTemplateType(title, attendeeCount, index, selfPath) {
    const norm = normalizeMeetingTitle(title);
    if (norm.length === 0)
        return inferTemplateType(title, attendeeCount);
    // Find the most-recent prior instance with the same normalized title.
    const prior = index
        .filter((m) => m.path !== selfPath &&
 ...
 */
function projectDocCandidates(brief) {
    const out = [];
    for (const section of brief.sections) {
        if (!PROJECT_DOC_HEADING_RE.test(section.heading))
            continue;
        for (const bullet of section.bullets) {
 ...
 */
export function renderCommitmentText(c) {
...
export function assembleAgendaScaffold(brief, attendees, template, opts = {}) {
    ...
    const attendeeCommitments = [];
    const attendeeOwnedIds = new Set();
    for (const att of attendees) {
        if (ownerSlug && att.slug.toLowerCase() === ownerSlug)
            continue;
        for (const c of att.commitments ?? []) {
            if (c.status !== 'open')
                continue;
            const idPrefix = c.id.slice(0, 8);
            if (attendeeOwnedIds.has(idPrefix))
                continue;
    ...
    const crossCutting = haveAttendeeCommitments
    ...
    // isn't emitted twice.
    const recentMeetingRels = new Set(recentMeetings.map((c) => bulletRelPath(c.text)).filter((r) => r !== null));
    ...
    const unrouted = [];
    if (!consumed.projectDoc)
        unrouted.push(...projectDocs);
...
export function renderScaffoldMarkdown(scaffold) {
    ...
    if (scaffold.crossCutting.length > 0) {
        ...
        lines.push('');
        lines.push("_The owner's own / org-wide open commitments that touch this person's" +
