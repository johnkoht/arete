 ...
 */
export function buildActionItemPrompt(content, personName) {
...
const VALID_ACTION_ITEM_DIRECTIONS = new Set(['i_owe_them', 'they_owe_me']);
/**
 ...
 */
export function parseActionItemResponse(response) {
    const trimmed = response.trim();
    if (!trimmed)
        return [];
    let jsonStr = trimmed;
    ...
    for (const item of raw.action_items) {
        ...
        if (!VALID_ACTION_ITEM_DIRECTIONS.has(direction))
            continue;
        results.push({ text, direction: direction });
    }
    return results;
}
// ---------------------------------------------------------------------------
 ...
 */
function extractActionItemsRegex(content, personName, source, date, ownerName) {
    const items = [];
 ...
 */
export async function extractActionItemsForPerson(content, personName, source, date, callLLM, ownerName) {
    if (!callLLM) {
