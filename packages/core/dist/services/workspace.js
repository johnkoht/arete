...
export class WorkspaceService {
     ...
     */
    async regenerateRootFiles(config, workspacePaths, options = {}) {
        ...
        const memory = adapter.supportsMemoryInjection !== undefined && adapter.supportsMemoryInjection()
    ...
    async update(workspaceRoot, options = {}) {
        ...
        for (const [filename, status] of Object.entries(regenResult)) {
            if (status === 'updated' || status === 'unchanged') {
                if (!result.updated.includes(filename))
