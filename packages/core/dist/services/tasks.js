 ...
 */
function formatTask(text, metadata, completed = false) {
    const checkbox = completed ? '[x]' : '[ ]';
    const parts = [text];
    if (metadata.area)
        parts.push(`@area(${metadata.area})`);
    if (metadata.project)
        parts.push(`@project(${metadata.project})`);
    if (metadata.person)
        parts.push(`@person(${metadata.person})`);
    if (metadata.from)
 ...
 */
function extractTasksFromSection(lines, section, file, sectionHeader) {
    const tasks = [];
...
export class TaskService {
    ...
    tasksFile;
    commitments;
    constructor(storage, workspacePaths, commitments) {
    ...
    async listTasks(options) {
        ...
        const weekDestinations = destinations.filter((d) => DESTINATION_MAP[d].file === 'week.md');
        const tasksDestinations = destinations.filter((d) => DESTINATION_MAP[d].file === 'tasks.md');
     ...
     */
    async addTask(text, destination, metadata = {}) {
        ...
        if (metadata.from?.type === 'commitment' && metadata.from.id) {
            ...
            const existing = existingTasks.find((t) => t.metadata.from?.type === 'commitment' && t.metadata.from.id === commitmentId);
            if (existing)
                return existing;
        }
     ...
     */
    async hasOpenTaskReferenceToCommitment(commitmentIdPrefix) {
        const openTasks = await this.listTasks({ completed: false });
        return openTasks.some((t) => t.metadata.from?.type === 'commitment' &&
            t.metadata.from.id === commitmentIdPrefix);
    }
    /**
