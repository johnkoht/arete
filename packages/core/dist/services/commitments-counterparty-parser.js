 ...
 */
const ARROW_PATTERN = /\[?@([a-z0-9-]+)\s*(→|←|->|<-)\s*@([a-z0-9-]+)\s*\]?/i;
/**
 ...
 */
const NL_PREPOSITION_PATTERN = /\b(to|from|with|for)\s+([A-Z][a-z]+(?:-[A-Z][a-z]+)?)\b/g;
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
function roleForDirection(direction) {
 ...
 * "Lindsay Gray").
 */
const CONJUNCTION_CONTINUATION_PATTERN = /^\s*(?:,\s*and\s+|,\s*|\s+and\s+|\s+&\s+)([A-Z][a-z]+(?:-[A-Z][a-z]+)?)\b/;
/**
 ...
 */
function extractNaturalLanguageNames(text, directory) {
    const out = [];
    const seen = new Set();
 ...
 */
function makeStakeholder(slug, role) {
    return { slug, role };
}
// ---------------------------------------------------------------------------
// Public API
 ...
 */
export function extractCounterpartiesFromText(text, ownerSlug, direction, directory) {
    ...
    for (const name of names) {
        ...
        if (slug === ownerKey)
            continue; // never include the owner
 ...
 *   //   }
 */
export function buildPersonDirectory(people) {
    const map = new Map();
...
//# sourceMappingURL=commitments-counterparty-parser.js.map