 ...
 */
export async function writeMeetingApplyFrontmatter(fm, intelligence, status, aliasDeps = {}) {
    ...
    const canAlias = !aliasDeps.skipTopicAlias &&
    ...
    if (canAlias) {
        try {
            ...
            const aliasResults = await aliasDeps.topicMemory.aliasAndMerge(proposedTopics, existingIdentities, { callLLM: aliasDeps.callLLM });
    ...
    fm['my_commitments'] = intelligence.actionItems.filter((i) => i.direction === 'i_owe_them').length;
