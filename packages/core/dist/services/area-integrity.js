...
async function listMarkdown(storage, dir) {
    try {
...
async function listSubdirs(storage, dir) {
    try {
        if (!(await storage.exists(dir)))
            return [];
...
async function scanAreaDeclarations(storage, areasDir) {
...
function frontmatterArea(frontmatter) {
...
async function collectFrontmatterRefs(storage, dir, root, refs) {
    let scanned = 0;
    for (const filePath of await listMarkdown(storage, dir)) {
        ...
        if (value)
            refs.push({ value, file: relative(root, filePath) });
    }
    return scanned;
}
/** Collect project README refs (frontmatter + prose) for one projects subdir. */
async function collectProjectRefs(storage, projectsSubdir, root, refs) {
    let scanned = 0;
    for (const dir of await listSubdirs(storage, projectsSubdir)) {
        ...
        scanned++;
        const parsed = parseFrontmatter(content);
        const resolution = resolveProjectArea(parsed?.frontmatter ?? {}, parsed?.body ?? content);
        if (resolution.area) {
 ...
 */
export async function checkAreaIntegrity(storage, paths) {
    const root = paths.root;
    const areasDir = join(root, 'areas');
    // -- 1. Canonical slugs + alias declarations -----------------------------
    const { canonicalSlugs, declaredAliases } = await scanAreaDeclarations(storage, areasDir);
    ...
    const duplicateClaims = new Map(); // alias → claimant slugs
    const shadowingAliases = [];
    ...
    const duplicateAliases = [...duplicateClaims.entries()]
        .filter(([, areas]) => areas.length > 1)
        .map(([alias, areas]) => ({ alias, areas }))
        .sort((a, b) => a.alias.localeCompare(b.alias));
    const resolves = (value) => canonicalSet.has(value) || knownAliases.has(value);
    // -- 2. Reference surfaces ------------------------------------------------
    const refs = [];
    let scannedFiles = 0;
    scannedFiles += await collectFrontmatterRefs(storage, join(paths.resources, 'meetings'), root, refs);
    scannedFiles += await collectProjectRefs(storage, join(paths.projects, 'active'), root, refs);
    scannedFiles += await collectProjectRefs(storage, join(paths.projects, 'archive'), root, refs);
    scannedFiles += await collectFrontmatterRefs(storage, join(paths.resources, 'notes'), root, refs);
    scannedFiles += await collectFrontmatterRefs(storage, paths.goals, root, refs);
    scannedFiles += await collectFrontmatterRefs(storage, join(paths.memory, 'topics'), root, refs);
    // Group unresolved values.
    const danglingByValue = new Map();
    for (const ref of refs) {
        if (resolves(ref.value))
            continue;
        const files = danglingByValue.get(ref.value);
        if (files)
            files.push(ref.file);
        else
            danglingByValue.set(ref.value, [ref.file]);
    }
    const dangling = [...danglingByValue.entries()]
...
//# sourceMappingURL=area-integrity.js.map