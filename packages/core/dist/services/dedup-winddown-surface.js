 ...
 */
export function filterLogByDate(entries, isoDate) {
    return entries.filter((e) => e.iso.startsWith(isoDate));
}
const SHORT = 8;
const shortId = (id) => id.replace(/^canon_/, '').slice(0, SHORT);
/**
 ...
 */
export function formatDedupedTodaySection(entries) {
    ...
    for (const m of merges) {
        ...
        lines.push(`  → wrong? add \`[[unmerge: ${shortId(m.canonicalId)} ← ${shortId(m.newId)}]]\` below to split next winddown`);
    }
    return lines.join('\n');
}
/**
 ...
 */
export function formatPossiblyMergeableSection(entries) {
    const uncertain = entries.filter((e) => e.decision === 'UNCERTAIN');
    if (uncertain.length === 0)
        return '';
    const lines = [];
    lines.push(`### Possibly mergeable (${uncertain.length} pair${uncertain.length === 1 ? '' : 's'} — your call)`);
    lines.push('');
    for (const u of uncertain) {
        const reasoning = u.reasoning ? ` — ${u.reasoning}` : '';
        lines.push(`- ${shortId(u.newId)} may be the same as canonical ${shortId(u.canonicalId)} (jaccard ${u.jaccard}, ${u.llmTier}-tier UNCERTAIN)${reasoning}`);
        lines.push(`  → confirm merge in the per-meeting approval UI, or leave as-is to keep them distinct`);
    }
    return lines.join('\n');
}
/**
 ...
 */
export function formatDedupWinddownSections(entries, isoDate) {
...
//# sourceMappingURL=dedup-winddown-surface.js.map