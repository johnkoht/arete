...
function calculateRecencyBoost(dateStr) {
    ...
    try {
        ...
        if (diffDays < 0)
            return 0;
        if (diffDays <= RECENCY_30_DAYS)
            return RECENCY_30_BOOST;
        if (diffDays <= RECENCY_90_DAYS)
            return RECENCY_90_BOOST;
        return 0;
    }
    catch {
        return 0;
...
function applyRecencyBoost(score, dateStr) {
...
function buildRelevance(section, queryTokens) {
    const titleLower = section.title.toLowerCase();
    const bodyLower = section.body.toLowerCase();
...
function getMemoryTypeFromPath(filePath) {
    const fileName = filePath.split(/[/\\]/).pop() || '';
    if (fileName === 'decisions.md')
        return 'decisions';
    if (fileName === 'learnings.md')
        return 'learnings';
    if (fileName === 'agent-observations.md')
        return 'observations';
    return null;
}
function isInDateRange(dateStr, range) {
    if (!range)
        return true;
    if (range.start && dateStr < range.start)
        return false;
    if (range.end && dateStr > range.end)
        return false;
    return true;
}
...
export class MemoryService {
    ...
    async getTimeline(query, paths, range) {
        const queryTokens = tokenize(query);
        const items = [];
        ...
        if (memoryExists) {
            for (const [memType, fileName] of Object.entries(MEMORY_FILES)) {
                ...
                for (const section of sections) {
                    if (!section.date)
                        continue;
                    if (!isInDateRange(section.date, range))
                        continue;
        ...
        if (meetingsExist) {
            ...
            for (const meetingPath of meetingFiles) {
                ...
                if (baseName === 'index.md')
                    continue;
                const meetingDate = extractDateFromFilename(baseName);
                if (meetingDate && !isInDateRange(meetingDate, range))
                    continue;
                const content = await this.storage.read(meetingPath);
                if (content === null)
                    continue;
        ...
        if (conversationsExist) {
            ...
            for (const conversationPath of conversationFiles) {
                ...
                if (baseName === 'index.md')
                    continue;
                const conversationDate = extractDateFromFilename(baseName);
                if (conversationDate && !isInDateRange(conversationDate, range))
                    continue;
                const content = await this.storage.read(conversationPath);
                if (content === null)
                    continue;
