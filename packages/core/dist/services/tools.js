 ...
 */
async function getToolInfo(storage, toolPath) {
 ...
 */
export async function listTools(storage, toolsDir) {
    ...
    for (const toolPath of subdirs) {
        const def = await getToolInfo(storage, toolPath);
        results.push(def);
    }
    return results;
}
 ...
 * @param toolsDir - Resolved absolute path to the tools directory
 */
export async function getTool(storage, id, toolsDir) {
    const toolPath = join(toolsDir, id);
    const exists = await storage.exists(toolPath);
    if (!exists)
        return null;
    return getToolInfo(storage, toolPath);
}
