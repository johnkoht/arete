 ...
 */
export function peopleDirectoryFromMap(map) {
    ...
    for (const [slug, raw] of Object.entries(map)) {
        ...
        if (emails.length > 0)
            normalized.set(slug.toLowerCase(), emails);
    }
    return {
        emailsForSlug(slug) {
 ...
 */
export function isSuppressed(commitment, now = new Date()) {
...
export function computeSuppressUntil(now = new Date()) {
 ...
 * Exported for test introspection.
 */
export function extractArtifactNouns(text) {
 ...
 * Exported for test introspection.
 */
export function checkArtifactMatch(artifactNouns, message) {
    if (artifactNouns.length === 0)
        return true; // N/A — no named artifact
    ...
    for (const noun of artifactNouns) {
        if (haystack.includes(noun))
            return true;
        if (attachNames.some((n) => n.includes(noun)))
            return true;
    }
 ...
 * Exported for test introspection.
 */
export function inTemporalWindow(commitmentDate, sentAt) {
    ...
    if (Number.isNaN(commitStart) || Number.isNaN(sent))
        return false;
    if (sent < commitStart)
        return false; // pre-commitment
 ...
 */
export function findResolutionEvidence(commitment, sentMessages, peopleDir, now = new Date()) {
    ...
    const recipientEmailToSlug = new Map();
    for (const slug of commitment.recipientSlugs) {
        for (const email of peopleDir.emailsForSlug(slug)) {
            ...
            if (!recipientEmailToSlug.has(email))
                recipientEmailToSlug.set(email, slug);
        }
    }
    ...
    for (const msg of sentMessages) {
        if (!msg.sentAt)
            continue; // no timestamp → can't temporal-gate
 ...
 */
export function buildResolutionPrompt(commitment, candidates) {
    const artifactNouns = extractArtifactNouns(commitment.text);
    const lines = [];
    lines.push('You are deciding whether a Sent email is evidence that an outbound commitment was COMPLETED.');
    lines.push('');
 ...
 * Exported for test introspection.
 */
export function parseResolutionResponse(response, candidates) {
    const seen = new Map();
    for (const rawLine of response.split(/\r?\n/)) {
        const line = rawLine.trim();
        if (!line)
            continue;
        const m = line.match(/^(\d+)[.)\]:]\s*(HIGH|MEDIUM|LOW)\b\s*(?:[|\-—]\s*(.*))?$/i);
        if (!m)
            continue;
 ...
 */
export async function runResolutionCrossCheck(commitment, candidates, callConcurrent, tier = 'fast') {
 ...
 */
export function applyResolutionDecisions(candidates, decisions) {
    if (candidates.length === 0) {
        return { kind: 'ignore', reason: 'no-candidates' };
    }
    const byId = new Map();
    for (const c of candidates)
        byId.set(c.threadId, c);
    const pick = (conf) => {
        let best = null;
 ...
 */
export async function runResolutionPipeline(commitment, sentMessages, peopleDir, callConcurrent, options = {}) {
 ...
 */
export function commitmentToResolutionInput(c) {
...
//# sourceMappingURL=commitment-resolution-pipeline.js.map