...
function renderGlobalTruncation(brief) {
    if (!brief.truncated)
        return '';
...
// Mode formatters
// ---------------------------------------------------------------------------
export function formatPersonBriefMarkdown(brief) {
    const parts = [];
...
export function formatProjectBriefMarkdown(brief) {
    const parts = [];
    const headerLines = [`# Brief: ${brief.subject}`, ''];
    const meta = brief.metadata;
    const metaBits = [
...
export function formatAreaBriefMarkdown(brief) {
    const parts = [];
    const headerLines = [`# Brief: Area — ${brief.metadata.name}`, ''];
    if (brief.metadata.status)
        headerLines.push(`**Status:** ${brief.metadata.status}`, '');
    parts.push(headerLines.join('\n'));
    for (const section of brief.sections)
        parts.push(renderSection(section));
    parts.push(renderSources(brief.sources));
...
export function formatMeetingBriefMarkdown(brief) {
    const parts = [];
    const headerLines = [`# Brief: ${brief.subject}`, ''];
    const meta = brief.metadata;
    const metaBits = [
...
//# sourceMappingURL=brief-formatters.js.map