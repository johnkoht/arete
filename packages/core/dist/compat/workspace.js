...
export function isAreteWorkspace(dir) {
    if (existsSync(join(dir, 'arete.yaml')))
        return true;
    const hasIde = existsSync(join(dir, '.cursor')) || existsSync(join(dir, '.claude'));
 ...
 */
export function parseSourceType(source, packageRoot) {
    if (source === 'npm') {
        return { type: 'npm', path: null };
    }
    if (source === 'symlink') {
        if (!packageRoot) {
            throw new Error('parseSourceType: packageRoot required when source is symlink');
        }
 ...
 */
export function getSourcePaths(packageRoot) {
