...
import type { SkillIntegration, SkillIntegrationOutputType, SkillDefinition } from '../models/index.js';

const INTEGRATION_START = '<!-- ARETE_INTEGRATION_START -->';
const INTEGRATION_END = '<!-- ARETE_INTEGRATION_END -->';

function defaultPath(type: SkillIntegrationOutputType): string {
  switch (type) {
 ...
 */
export function generateIntegrationSection(
  skillId: string,
 ...
 */
export function injectIntegrationSection(
  ...
  if (section === null) {
    if (!hasMarkers) return skillMdContent;
 ...
 */
export function deriveIntegrationFromLegacy(
  def: SkillDefinition
): SkillIntegration | undefined {
  if (def.createsProject !== true) return undefined;
