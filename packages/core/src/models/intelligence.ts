...
// ---------------------------------------------------------------------------
// Phase 9 — Typed-mode brief primitives
//
// Pure aggregator output: structured per-mode briefs that the agent can
// consume to synthesize a deliverable (agenda, status, talking-points).
// No LLM inside the assembler — these types describe deterministic gather.
// ---------------------------------------------------------------------------
...
export type AttendeeMiniBrief = {
  ...
  resolved: boolean;
  highlights?: string[];
  recentMeetings?: Array<{ title: string; date: string; path: string }>;
...
export type MeetingBrief = BriefBase & {
  ...
  metadata: {
    ...
    inferredArea?: { slug: string; confidence: number };
    explicitArea?: string;
  };
