...
import { writeWithLock, type WriteWithLockResult } from './meeting-lock.js';
 ...
 */
export function buildReverseStampMarker(
  newMeetingSlug: string,
  newMeetingDate: string,
): string {
 ...
 * Exported for tests.
 */
export function matchReverseStampMarker(
  text: string,
  newMeetingSlug: string,
): boolean {
 ...
 * Exported for tests.
 */
export function insertReverseStampIntoBody(
  body: string,
  marker: string,
  newMeetingSlug: string,
  itemId?: string,
 ...
 */
export async function applyReverseStamp(
  storage: StorageAdapter,
  request: ReverseStampRequest,
): Promise<ReverseStampResult> {
  const marker = buildReverseStampMarker(
    request.newMeetingSlug,
    request.newMeetingDate,
  );

  let result: WriteWithLockResult;
  try {
    result = await writeWithLock(
      storage,
      request.canonicalMeetingPath,
      async ({ body }) => {
        const { body: nextBody, changed } = insertReverseStampIntoBody(
          body,
          marker,
          request.newMeetingSlug,
