...
export type BackgroundDedupScope =
  | 'commitments'
  | 'decisions'
  | 'learnings'
  | 'topics';
...
export type RunBackgroundDedupInputs = {
  ...
  // -------------------------------------------------------------------------
  // Scope-specific inputs (only the relevant one is used per scope)
  // -------------------------------------------------------------------------
 ...
 */
export async function runBackgroundDedup(
  inputs: RunBackgroundDedupInputs,
): Promise<BackgroundDedupResult> {
  switch (inputs.scope) {
    case 'commitments':
    ...
    default: {
      ...
      const _exhaustive: never = inputs.scope;
 ...
 */
async function runCommitmentsScope(
  inputs: RunBackgroundDedupInputs,
): Promise<BackgroundDedupResult> {
  ...
  const filtered = inputs.commitments.filter((c) => {
    ...
    if (inputs.since && c.date.slice(0, 10) < inputs.since) return false;
    return true;
  });
  ...
  const groups = new Map<string, BackgroundDedupGroup>();
  const candidates: BackgroundDedupCandidatePair[] = [];
  const merged = new Set<string>(); // ids already absorbed into a group
  ...
  for (let i = 0; i < ordered.length; i += 1) {
    ...
    if (laterRows.length === 0) continue;

    const canonicalDedupInput = commitmentToDedupInput(subject);
    const canonicalAsExisting: ExistingCommitmentForDedup = canonicalDedupInput;

    for (const later of laterRows) {
      if (merged.has(later.id)) continue;
      const adapted: ExtractedItemForDedup = {
        id: later.id,
        text: later.text,
      ...
      let llmDecisions: LLMPairDecision[] = [];
      if (inputs.callConcurrent) {
        llmDecisions = await runLLMCrossCheck(
          adapted,
          found.candidates,
          inputs.callConcurrent,
          inputs.tier ?? 'fast',
        );
      }

      if (inputs.callConcurrent && llmDecisions.length > 0) {
        const outcome = applyDedupDecisions(
          adapted,
          found.candidates,
          llmDecisions,
        );
  ...
  const summary: BackgroundDedupSummary = {
    scope: 'commitments',
 ...
 * mechanical).
 */
async function runMemorySectionsScope(
  inputs: RunBackgroundDedupInputs,
): Promise<BackgroundDedupResult> {
  if (!inputs.sections) {
    throw new Error(
      `scope='${inputs.scope}' requires \`sections\` input`,
    );
  }
  ...
  for (let i = 0; i < ordered.length; i += 1) {
    ...
    if (merged.has(left.title)) continue;
    ...
    for (let j = i + 1; j < ordered.length; j += 1) {
      ...
      if (merged.has(right.title)) continue;
  ...
  const groupsArr = [...groups.values()];
  const summary: BackgroundDedupSummary = {
    scope: inputs.scope as 'decisions' | 'learnings',
    totalIn: filtered.length,
    groups: groupsArr.length,
    duplicates: groupsArr.reduce((n, g) => n + g.duplicates.length, 0),
...
async function runTopicsScope(
  ...
  const surfaces = filtered.map((t) => {
    ...
    for (const a of t.aliases) s.add(a.toLowerCase());
    return s;
  });
  ...
  for (let i = 0; i < filtered.length; i += 1) {
    ...
    for (let j = i + 1; j < filtered.length; j += 1) {
      ...
      const j_score =
        leftTokens.size > 0 && rightTokens.size > 0
  ...
  const groups: BackgroundDedupGroup[] = [];
  const summary: BackgroundDedupSummary = {
    scope: 'topics',
    totalIn: filtered.length,
    groups: 0,
...
function ensureGroup(
  groups: Map<string, BackgroundDedupGroup>,
  canonicalKey: string,
  canonicalText: string,
): BackgroundDedupGroup {
  const existing = groups.get(canonicalKey);
  if (existing) return existing;
  const fresh: BackgroundDedupGroup = {
    canonicalKey,
    canonicalText,
    duplicates: [],
  };
 ...
 */
function normalizeMemoryText(body: string): string {
...
export function formatBackgroundDedupDiff(args: {
  ...
  if (args.groups.length > 0) {
    ...
    for (const g of args.groups) {
      ...
      for (const d of g.duplicates) {
        ...
        lines.push(
          `  - ${truncate(d.key, 64)} (jaccard ${d.jaccard.toFixed(2)})${llm}${reasoning}`,
        );
  ...
  if (args.candidates.length > 0) {
    ...
    for (const p of args.candidates) {
      ...
      lines.push(
        `- ${truncate(p.leftKey, 48)} ⇄ ${truncate(p.rightKey, 48)} (jaccard ${p.jaccard.toFixed(2)})${reasoning}`,
      );
 ...
 */
export function applyCommitmentsDedup(
  commitments: ReadonlyArray<Commitment>,
  result: BackgroundDedupResult,
): Commitment[] {
  const byId = new Map<string, Commitment>();
  for (const c of commitments) byId.set(c.id, c);

  // Build a map of duplicate-id → canonical-id for fast lookup.
  const dupeToCanonical = new Map<string, string>();
  const canonicalSourceMeetings = new Map<string, Set<string>>();
  const canonicalTextVariants = new Map<string, string[]>();

  for (const group of result.groups) {
    const canonical = byId.get(group.canonicalKey);
    if (!canonical) continue;
    const sm = new Set<string>(canonical.source_meetings ?? []);
    if (canonical.source) sm.add(canonical.source);
    ...
    for (const dup of group.duplicates) {
      ...
      if (!dupCommitment) continue;
      // Union source_meetings.
      if (dupCommitment.source_meetings) {
        for (const s of dupCommitment.source_meetings) sm.add(s);
      }
      if (dupCommitment.source) sm.add(dupCommitment.source);
  ...
  for (const c of commitments) {
    if (dupeToCanonical.has(c.id)) continue; // duplicate — skip
 ...
 */
export function collectDupeProvenance(
  commitments: ReadonlyArray<Commitment>,
  result: BackgroundDedupResult,
  ...
  const payloads: DedupDecisionLogPayload[] = [];
  for (const group of result.groups) {
    if (!byId.has(group.canonicalKey)) continue;
    for (const dup of group.duplicates) {
      const dupCommitment = byId.get(dup.key);
      if (!dupCommitment) continue;
      const sourceMeeting =
        dupCommitment.source ?? dupCommitment.source_meetings?.[0];
      if (!sourceMeeting) continue; // no resolvable source → skip (no half-record)
      payloads.push({
        decision: 'MERGE',
        newId: dup.key,
