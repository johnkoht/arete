...
function parseDecisionEntries(content: string): Array<{ title: string; date?: string; body: string; raw: string }> {
  const entries: Array<{ title: string; date?: string; body: string; raw: string }> = [];
...
function parseLearningEntries(content: string): Array<{ date: string; text: string; raw: string }> {
  const entries: Array<{ date: string; text: string; raw: string }> = [];
...
export class HygieneService {
  constructor(
    ...
    private readonly areaMemory: AreaMemoryService,
  ...
  async scan(options?: HygieneScanOptions): Promise<HygieneReport> {
    ...
    const shouldScan = (tier: HygieneTier, category: HygieneCategory): boolean => {
      if (tiers && !tiers.includes(tier)) return false;
    ...
    if (shouldScan(1, 'meetings')) {
      ...
      if (meetingsDirExists) {
        ...
        for (const filePath of files) {
          ...
          if (!status || !ARCHIVABLE_STATUSES.has(status)) continue;

          const dateStr = fm.date;
          if (!dateStr) continue;

          const age = daysAgo(dateStr, now);
          if (age < meetingOlderThanDays) continue;
    ...
    if (shouldScan(1, 'commitments')) {
      ...
      if (commitmentsContent) {
        try {
          const parsed = JSON.parse(commitmentsContent) as { commitments?: Array<{ id: string; text: string; status: string; resolvedAt: string | null; date: string }> };
          const commitmentsList = Array.isArray(parsed.commitments) ? parsed.commitments : [];
          const resolved = commitmentsList.filter(c =>
            (c.status === 'resolved' || c.status === 'dropped') && c.resolvedAt
          );

          for (const c of resolved) {
            const age = daysAgo(c.resolvedAt!, now);
            if (age < commitmentOlderThanDays) continue;
    ...
    if (shouldScan(2, 'memory')) {
      ...
      if (decisionsContent) {
        ...
        for (const entry of entries) {
          ...
          if (age < memoryOlderThanDays) continue;
    ...
    if (shouldScan(3, 'memory')) {
      ...
      const allEntries: Array<{ id: string; text: string; type: string; path: string }> = [];
    ...
    // Build summary
    const byTier: Record<HygieneTier, number> = { 1: 0, 2: 0, 3: 0 };
    const byCategory: Record<HygieneCategory, number> = {
      meetings: 0,
      memory: 0,
      commitments: 0,
      activity: 0,
  ...
  async apply(report: HygieneReport, actions: ApprovedAction[]): Promise<HygieneResult> {
    // Validate freshness
    const scannedAt = new Date(report.scannedAt);
    const now = new Date();
    if (now.getTime() - scannedAt.getTime() > STALE_REPORT_MS) {
      throw new Error(
        `Scan report is stale (scanned at ${report.scannedAt}). Please re-scan before applying.`,
      );
    }
  ...
  private async purgeCommitments(item: HygieneItem): Promise<void> {
    const olderThanDays = (item.metadata.ageDays as number) ?? DEFAULT_COMMITMENT_OLDER_THAN_DAYS;
  ...
  private async compactMemory(item: HygieneItem): Promise<void> {
    const memoryType = item.metadata.type as string;
  ...
  private async trimActivity(item: HygieneItem): Promise<void> {
    const absPath = join(this.workspaceRoot, item.affectedPath);
    const content = await this.storage.read(absPath);
    if (!content) throw new Error(`File not found: ${item.affectedPath}`);
