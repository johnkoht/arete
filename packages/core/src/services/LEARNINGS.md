...
- **`parseMemorySections()` is a single-pass classifier with explicit priority order + code-fence tracking** (2026-04-27, wiki-leaning-extraction): The parser in `services/memory.ts` must classify each line into exactly ONE header shape, in this priority order, with NO fall-through: (1) `## Title` (new writer convention from Task 1+), (2) `### YYYY-MM-DD: Title` (legacy date-prefixed), (3) `### Title` (legacy bare). First match wins. The structure exists because `## Title` and `### Title` would double-match if tried independently and the legacy dated form would be mis-classified as bare if the dated regex ran second — the priority order is load-bearing for R1 mitigation against real `learnings.md` / `decisions.md` corpora that mix all three shapes. **Code-fence tracking** (`inFence` toggle on lines matching `/^```/`) is mandatory: headers inside fenced code blocks must NOT open sections, otherwise example markdown inside doc-comments or pasted code corrupts the section list. **Empty/whitespace titles are rejected** in `classifyHeader()` — a stray `## ` line doesn't open a phantom section. **Metadata bullets** (`- **Date|Source|Topics**:`) attach to the most-recently-opened section; orphan bullets that appear before any header are discarded (no preamble stub emitted). **Topics absent vs. empty distinction is preserved**: missing bullet → `topics === undefined`; bullet with all empty/whitespace entries → `topics === undefined`; non-empty list → `topics: string[]`. If you change any of: priority order, fall-through behavior, fence tracking, empty-title rejection, or metadata-attribution scope — the canonical regression suite is `packages/core/test/services/memory.test.ts` (`parseMemorySections` suite, 19 tests) and it MUST continue to pass.

- **Cross-person bilateral dedup in `refreshPersonMemory()`** (2026-04-09): When extraction captures both sides of a bilateral action item (`@owner → @counterparty`), the per-person hash dedup can't catch it because `personSlug` is part of the hash. Post-loop cross-person dedup in `refreshPersonMemory()` suppresses the owner's self-reminder copy when matching text+source exists under any counterparty. Fix is at the entity-service level (not the parser) because the parser correctly produces both items — the duplication is an artifact of the per-person iteration model. Scoped to same-source (meeting file) to prevent cross-meeting false positives.
...
- **Action-item fragmentation is a prompt problem, not a dedup problem** (2026-04-17): Post-extraction Jaccard (0.8 threshold) in `meeting-processing.ts` catches near-identical text, not semantic duplicates. Four fragmentation patterns must be consolidated upstream in `buildMeetingExtractionPrompt()`: (1) **collaborative-initiative split** — one pilot with multiple contributors became N items; (2) **handoff chain** — A identifies, B agrees, C picks up = N items instead of one owned by the actual doer; (3) **narrative obsolescence** — mid-meeting commitments resolved later in the same transcript were still extracted; (4) **speculation-as-commitment** — "I wonder if…"/"Maybe we could…" framed as action items. The fix: a "Consolidation" section in the prompt with a self-review test ("if completing X would also complete Y, merge them") + explicit patterns + negative examples. The "enabling sub-tasks fold in" rule (get access/provision/send test data → fold into parent) was the single most load-bearing addition — without it, models reliably carve admin sub-tasks off as their own items. Stage-1 narrative-map architecture is deferred; prompt-only fix at frontier tier handled all 4 patterns. Kept in `packages/core/src/services/meeting-extraction.ts` around L660 ("## Consolidation — emit ONE item per unit of work").

- **Extraction tier is user-scoped in `arete.yaml`, not framework-default** (2026-04-17): `DEFAULT_TASK_TIERS.extraction = 'fast'` in `services/ai.ts` stays put. Workspaces opt into frontier (opus) per-project in `ai.tasks.extraction: frontier`. Rationale: Opus is ~3–5× cost/latency vs Sonnet; justified only where meeting volume is low and consolidation matters. Framework default remains the cheap tier. **Baseline gotcha**: a passing regression regex proves nothing if the extractor didn't extract the category at all — Sonnet "passed" the glance case-sensitivity check on the first baseline only because it missed the bug entirely (false negative masquerading as pass). Always sanity-check base rates before declaring a regression "works."

- **Body-only hashing for meeting-source idempotency** (2026-04-23, topic-wiki-memory): `hashSource(full-content)` treats frontmatter edits as semantic changes — so adding an attendee, bumping `processed_at`, or re-running extraction busts dedup and forces re-integration on every refresh. The fix is `hashMeetingSource(content)` in `services/topic-memory.ts` — parses out frontmatter via `/^---\r?\n[\s\S]*?\r?\n---\r?\n?/` and hashes only the body. Use this any time you're caching "did we already process this source file?" where the source has YAML frontmatter that can be touched by unrelated writes (processing status, metadata). Don't hash the full file unless frontmatter IS part of the semantic payload.

- **Structured LLM output: enum keys + length caps + frontmatter-injection guard** (2026-04-23, topic-wiki-memory): Any time an LLM produces JSON that's written into a YAML-frontmattered markdown file, the output schema must enforce: (1) **enum-keyed unions** not `Record<string, string>` — otherwise the LLM can typo section names and silently erase content under the wrong key, (2) **per-value length caps** — otherwise the LLM can echo its entire input into one field (saw this at ~10KB per section with Haiku on retries), (3) **reject any value containing raw `---`** — a section body with a triple-dash terminator breaks frontmatter parsing on the next read. See `parseIntegrateResponse` in `topic-memory.ts`. Zod's permissive `Record<string,string>` isn't strict enough; the eng-lead review flagged this before we shipped.

- **"Services tested" ≠ "services wired"** (2026-04-23, topic-wiki-memory): Shipped `aliasAndMerge` and `renderActiveTopicsAsSlugList` as tested-but-never-called service methods. The plan was marked `status: completed` at Step 10; the end-to-end reviewer caught both as dark code. **Before declaring a plan done, grep every exported symbol added in the branch for non-test callers.** If the only hits are self + tests, flag it. The `export` keyword + green tests is a seductive signal — the actual signal is "production call site exists and exercises this path." Documented this as a pattern in the `2026-04-23_topic-wiki-memory-learnings.md` entry.

- **Advisory lock at outer CLI boundary; service respects `skipLock`** (2026-04-23, topic-wiki-memory): When a CLI command holds a lock across multiple service calls (`arete topic seed` acquires `.arete/.seed.lock` outside its batch helper), the service helper should take an explicit `skipLock?: boolean` option. Seed passes `skipLock: true`; every other caller (`arete memory refresh`, `arete topic refresh`) lets the service acquire. This avoids EEXIST-on-self and keeps the locking primitive reentrant at the command level, not the call-stack level. Pattern is in `topic-memory.ts:refreshAllFromMeetings`. The first shipped version was asymmetric (seed had lock, refresh didn't) — reviewer caught it in round 2.

- **Regression regexes must match verbs, not keywords** (2026-04-17): When encoding fragmentation patterns as regex watchdogs (e.g., in a local eval harness), anchor on the action verb, not just a keyword. `/claude|damage.*(?:estimat|photo)/i` false-fires on correctly-consolidated items that merely mention Claude. `/\b(pilot|test|investigat|review|evaluat)\s+claude\b/i` catches only items whose primary action is Claude-pilot work. Same principle for "get access" vs consolidated-pilot-that-mentions-access: use `^(get|provision|obtain|request)\s+claude\s+(access|license)` with a start-anchor. Loose keyword regexes produce false positives the moment the fix works.
