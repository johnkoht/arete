...
- **`MemoryService.search()` now includes meetings and conversations, not just memory items** (2026-02-26): Prior to this fix, `arete memory search` only searched `.arete/memory/items/` (decisions.md, learnings.md, agent-observations.md). It silently ignored `resources/meetings/` and `resources/conversations/` even though `context.ts` categorizes both as "memory" content. Now `search()` scans all three sources and returns results with `type: 'meeting'` or `type: 'conversation'` as appropriate. The `ExtendedMemoryItemType` type (`MemoryItemType | 'meeting' | 'conversation'`) is used in `MemoryResult.type` and `TimelineItem.type`. Tests added in `packages/core/test/services/memory.test.ts`.
