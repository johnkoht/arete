 ...
 */
const VERB_LEMMAS: ReadonlyMap<string, string> = new Map([
  // talk
  ['talked', 'talk'],
  ['talking', 'talk'],
  ['talks', 'talk'],
 ...
 * over "will".
 */
const LEADING_INTENT_PREFIXES: ReadonlyArray<string[]> = [
  ['ill', 'go', 'ahead', 'and'],
  ['i', 'will', 'go', 'ahead', 'and'],
  ['i', 'am', 'going', 'to'],
  ['im', 'going', 'to'],
 ...
 * is noise.
 */
const MULTI_TOKEN_FORMS: ReadonlyArray<[string[], string]> = [
  [['will', 'send'], 'send'],
  [['will', 'talk'], 'talk'],
 ...
 */
function stripArrowNotation(text: string): string {
  let out = text;
 ...
 */
function stripLeadingIntent(tokens: string[]): string[] {
 ...
 */
function collapseMultiTokenForms(tokens: string[]): string[] {
  const out: string[] = [];
  let i = 0;
 ...
 * variants stable.
 */
function stripSlugMentions(tokens: string[]): string[] {
 ...
 */
export function normalizeCommitmentTextV2(text: string): string {
  if (!text) return '';
 ...
 */
export function computeCommitmentHashV2(
  text: string,
