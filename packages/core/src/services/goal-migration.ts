 ...
 */
export function extractQuarter(content: string): string {
 ...
 */
function parseFormatA(content: string, quarter: string): ParsedGoal[] {
 ...
 */
function parseFormatB(content: string): ParsedGoal[] {
  const goals: ParsedGoal[] = [];
 ...
 */
function generateFrontmatter(goal: ParsedGoal, quarter: string): string {
  const lines = [
    '---',
    `id: "${goal.id}"`,
...
export class GoalMigrationService {
   ...
   */
  async migrate(workspaceRoot: string): Promise<GoalMigrationResult> {
