 ...
 */
export function filterSameDayOpenCommitments(
  ...
  for (const c of commitments) {
    ...
    if (c.date.slice(0, 10) !== meetingDate.slice(0, 10)) continue;
 ...
 */
export async function runExtractDedup(
  extractedItems: ReadonlyArray<ExtractedItemForExtractDedup>,
  inputs: ExtractDedupInputs,
  callConcurrent: LLMCallConcurrentFn,
  options: { tier?: 'fast' | 'standard' | 'frontier' } = {},
  ...
  const sameDayCommitments = filterSameDayOpenCommitments(
  ...
  const existingPool: ExistingCommitmentForDedup[] = [
    ...sameDayCommitments,
  ...
  for (const item of extractedItems) {
    ...
    const { outcome, candidates, decisions: llmDecisions } = await runDedupPipeline(
      adapted,
      existingPool,
      callConcurrent,
      { tier, sameDay: true },
 ...
 */
export function decorateStagedSectionsWithDupeBadges(
 ...
 */
export function buildDupeSkipReasonEntries(
  decisions: ReadonlyArray<ExtractDedupDecision>,
  nowIso: string,
): Record<string, {
  reason: string;
 ...
 */
export function buildDupeStatusEntries(
  decisions: ReadonlyArray<ExtractDedupDecision>,
): Record<string, 'skipped'> {
  const out: Record<string, 'skipped'> = {};
  for (const d of decisions) {
    if (d.outcome.kind === 'definite-dupe') {
