...
export class WorkspaceService {
    ...
    if (sourcePaths && this.storage.copy) {
          ...
          for (const src of profileFiles) {
            ...
            if (!(await this.storage.exists(dest))) {
              try {
                ...
                result.errors.push({ type: 'file', path: join('.agents', 'profiles', filename), error: (err as Error).message });
              }
            }
          }
        }
      ...
      if (await this.storage.exists(sourcePaths.rules)) {
        ...
        for (const src of ruleFiles) {
          if (!getProductRulesAllowList(ideTarget).some((r) => src.endsWith(r))) continue;
  ...
  async update(
    ...
    const paths: WorkspacePaths = options.ideTarget
      ? {
          root: workspaceRoot,
          manifest: join(workspaceRoot, 'arete.yaml'),
          ideConfig: join(workspaceRoot, adapter.configDirName),
    ...
    if (options.sourcePaths?.skills) {
      ...
      try {
        ...
        if (migrated.cleaned.length > 0) {
          if (!result.cleaned) result.cleaned = [];
    ...
    try {
      ...
      for (const file of seedResult.added) {
        if (!result.added.includes(file)) result.added.push(file);
      }
    ...
    if (typeof adapter.generateCommands === 'function') {
      ...
      if (await this.storage.exists(commandsDir)) {
        ...
        for (const cmd of existingCmds) {
          try { await this.storage.delete(cmd); } catch { /* non-fatal */ }
        }
      }
      const commands = adapter.generateCommands(skills);
      for (const [filename, content] of Object.entries(commands)) {
    ...
    if (options.sourcePaths?.rules) {
      ...
      for (const src of ruleFiles) {
        if (!allowList.some((r) => src.endsWith(r))) continue;
        const baseName = src.split(/[/\\]/).pop() ?? '';
        const destName = baseName.replace(/\.mdc$/, adapter.ruleExtension);
        const dest = join(paths.rules, destName);
        if (!(await this.storage.exists(dest))) {
   ...
   */
  private async ensureManagedSkillMd(
