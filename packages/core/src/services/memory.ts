...
function finalizeSection(p: PendingSection): MemorySection {
  return {
    title: p.title,
    date: p.date,
    source: p.source,
 ...
 */
function classifyHeader(line: string): { title: string; date?: string } | null {
    ...
    if (current) {
      ...
      if (topicsMatch && current.topics === undefined) {
        const parts = topicsMatch[1]
          ...
          .map((p) => p.trim())
          .filter((p) => p.length > 0);
        if (parts.length > 0) current.topics = parts;
      }
      current.bodyLines.push(line);
      current.rawLines.push(line);
    }
 ...
 */
function computeSinceCutoff(sinceDays: number, now: Date = new Date()): string {
 ...
 * it directly.
 */
export async function getMemoryItemsForTopics(
  paths: string[],
  topicSlugs: string[],
  opts?: { limit?: number; sinceDays?: number },
): Promise<MemoryEntry[]> {
  if (paths.length === 0 || topicSlugs.length === 0) return [];
  ...
  for (const slug of topicSlugs) perSlugCounts.set(slug, 0);
  ...
  for (const filePath of paths) {
    ...
    for (const section of sections) {
      if (!section.topics || section.topics.length === 0) continue;
      ...
      if (section.date && section.date < cutoff) continue;
      ...
      // emit time.
      const slugsWithCapacity = matchedSlugs.filter(
        (s) => (perSlugCounts.get(s) ?? 0) < limit,
      );
      if (slugsWithCapacity.length === 0) continue;
