...
const DIRECTIVE_PATTERN =
  /\[\[(unskip|confirm-skip)\s+(?:([a-z0-9-]+):)?(ai_\d+|de_\d+|le_\d+)\]\]/gi;
 ...
 */
export function parseChefSkipDirectives(content: string): ChefSkipDirective[] {
  const results: ChefSkipDirective[] = [];
  ...
  while ((match = DIRECTIVE_PATTERN.exec(content)) !== null) {
    ...
    const kind = kindRaw.toLowerCase() as ChefSkipDirectiveKind;
    results.push({
      kind,
      id,
      slug: slugRaw ?? null,
 ...
 */
export async function resolveChefSkipDirective(
  storage: StorageAdapter,
  directive: ChefSkipDirective,
  options: ResolveOptions,
): Promise<ResolvedDirective> {
 ...
 */
export function formatDirectiveStatusMessage(d: ResolvedDirective): string | null {
  switch (d.status) {
    case 'resolved':
      return null;
    case 'ambiguous': {
      const slugs = d.candidates
        ...
        .join(', ');
      return (
        `[[${d.kind} ${d.id}]] is ambiguous — found in multiple meetings: ` +
        `${slugs}. Please qualify: \`[[${d.kind} <slug>:${d.id}]]\`.`
      );
    }
    case 'no-match':
      return (
        `[[${d.kind} ${d.id}]] — no match. The item may have already been ` +
        `processed (committed or cleared on apply), or the id may be a typo.`
      );
    case 'invalid-slug':
      return (
        `[[${d.kind} ${d.slug}:${d.id}]] — slug-qualified meeting was not ` +
