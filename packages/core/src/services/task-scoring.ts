...
export interface ScoreBreakdown {
  dueDate: { score: number; reason: string };
  commitment: { score: number; reason: string };
  meetingRelevance: { score: number; reason: string };
  weekPriority: { score: number; reason: string };
  modifiers: { score: number; reasons: string[] };
  total: number;
}
 ...
 */
export function scoreMeetingRelevance(
  task: WorkspaceTask,
  context: Pick<ScoringContext, 'todayMeetingAttendees' | 'todayMeetingAreas'>,
): { score: number; reason: string } {
 ...
 */
export function scoreWeekPriority(
  task: WorkspaceTask,
  weekPriorities: string[],
): { score: number; reason: string } {
...
export function calculateModifiers(
  ...
  const hasTodayMeeting =
    (person && context.todayMeetingAttendees.includes(person)) ||
 ...
 */
function isDeepWorkTask(task: WorkspaceTask): boolean {
 ...
 */
export function formatScoredTask(scoredTask: ScoredTask, rank: number): string {
 ...
 */
export function formatTaskRecommendations(
