 ...
 */
export type MigrationRowCategory =
  | 'pass-through'
  | 'collapsed'
  | 'self-rewrite'
  | 'status-conflict'
...
export type MigrationDiffRow = {
  ...
  hash: string;
  before: Commitment[]; // 1+ for collapsed/status-conflict; 1 for pass-through/self-rewrite
  after: Commitment | null; // null when ambiguous (still needs disambiguation)
 ...
 */
const ROLE_PRIORITY: Record<StakeholderRole, number> = {
  recipient: 3,
  sender: 3,
  mentioned: 2,
  self: 1,
...
function mergeStakeholderRoles(existing: Stakeholder, incoming: Stakeholder): Stakeholder {
...
function unionStakeholders(lists: ReadonlyArray<ReadonlyArray<Stakeholder>>): Stakeholder[] {
  const bySlug = new Map<string, Stakeholder>();
...
// ---------------------------------------------------------------------------
// Internal: parse each row + bucket
// ---------------------------------------------------------------------------

type ParsedRow = {
  source: Commitment;
  index: number;
  /** Inferred direction (may be 'self' after parser). */
  direction: CommitmentDirection;
  stakeholders: Stakeholder[];
  hash: string;
  ambiguous: boolean;
  ambiguousNames?: AmbiguousName[];
  /** True when the parser rewrote the direction from i_owe_them/they_owe_me → self. */
  selfRewritten: boolean;
};
...
function parseRow(
  ...
  const parsed = extractCounterpartiesFromText(
    c.text,
    inputs.ownerSlug,
    c.direction,
    inputs.directory,
  ...
  if (ambiguous && ambiguousNames && inputs.disambiguations) {
    ...
    const stillAmbiguous: AmbiguousName[] = [];
    for (const an of ambiguousNames) {
      ...
      if (chosen && an.candidates.includes(chosen)) {
        const role: StakeholderRole =
          direction === 'they_owe_me' ? 'sender' : 'recipient';
    ...
    if (stillAmbiguous.length === 0) {
      ...
      const merged: Stakeholder[] = [...stakeholders];
 ...
 */
function resolveGroupStatus(group: ReadonlyArray<Commitment>): {
  ...
  if (terminal.length > 0) {
    ...
    const status: Commitment['status'] = terminal.some(
      (c) => c.status === 'resolved',
    )
      ? 'resolved'
      : 'dropped';
...
function buildCanonicalV2(
  group: ReadonlyArray<ParsedRow>,
): { canonical: Commitment; conflicted: boolean } {
  ...
  const stakeholders = unionStakeholders(
    sorted.map((r) => r.stakeholders),
  );

  // source_meetings: canonical's `source` first, then any others.
  const sourceMeetings: string[] = [];
  ...
  for (const r of sorted) {
    ...
    seenText.add(t);
    textVariants.push(t);
    if (textVariants.length >= COMMITMENT_TEXT_VARIANTS_MAX) break;
  }

  // Status resolution.
  const { status, resolvedAt, conflicted } = resolveGroupStatus(
 ...
 */
export function migrateCommitmentsToV2(
  inputs: MigrationInputs,
): MigrationResult {
  const parsed: ParsedRow[] = inputs.commitments.map((c, idx) =>
    parseRow(c, idx, inputs),
  );
  ...
  for (const r of resolvable) {
    ...
    else buckets.set(r.hash, [r]);
  }

  const migrated: Commitment[] = [];
  const diff: MigrationDiffRow[] = [];
  ...
  for (const [hash, group] of bucketEntries) {
    ...
    const notes: string[] = [];
    let category: MigrationRowCategory;

    if (group.length > 1 && conflicted) {
      category = 'status-conflict';
      notes.push(
        `Mixed status across ${group.length} rows: ${[...new Set(group.map((r) => r.source.status))].join(', ')}; resolved to '${canonical.status}'`,
      );
      ...
      notes.push(
        `direction rewritten from '${group[0].source.direction}' to 'self' (no resolvable counterparty in text)`,
      );
    } else {
    ...
    if (distinctVariants > COMMITMENT_TEXT_VARIANTS_MAX) {
      notes.push(
        `${distinctVariants} distinct text variants → capped at ${COMMITMENT_TEXT_VARIANTS_MAX} (oldest-first drop)`,
      );
    }
 ...
 */
export function formatMigrationDiff(
  ...
  if (ambig.length > 0) {
    ...
    lines.push('');
    lines.push(
      'Edit `.arete/commitments.pre-phase-10-ambiguities.json` to specify the chosen slug for each entry below, then re-run `--dry-run` to verify.',
    );
    lines.push('');
    for (const row of ambig) {
      ...
      if (row.ambiguous) {
        for (const an of row.ambiguous) {
          lines.push(
            `- Name "${an.name}" matches: ${an.candidates.map((s) => `\`${s}\``).join(', ')}`,
          );
        }
      }
      lines.push('');
  ...
  const sectionOrder: MigrationRowCategory[] = [
    'collapsed',
    'status-conflict',
    'self-rewrite',
    'pass-through',
  ...
  for (const cat of sectionOrder) {
    ...
    if (cat === 'pass-through') {
      for (const row of rows) {
        const c = row.before[0];
        lines.push(
          `- \`${c.id.slice(0, 8)}\` \`${shortHash(row.hash)}\` ${quoteText(c.text)}`,
        );
      }
      lines.push('');
      continue;
    ...
    for (const row of rows) {
      ...
      for (const b of row.before) {
        lines.push(
          `  - \`${b.id.slice(0, 8)}\` ${b.source} ${b.date} ${b.status} dir=${b.direction} personSlug=${b.personSlug}`,
        );
      }
      if (row.after) {
        lines.push(`- AFTER:`);
        ...
        lines.push(
          `  - stakeholders: ${row.after.stakeholders?.map((s) => `${s.slug}/${s.role}`).join(', ') ?? '(none)'}`,
        );
        lines.push(
          `  - source_meetings: ${row.after.source_meetings?.join(', ') ?? '(none)'}`,
        );
        lines.push(
          `  - textVariants (${row.after.textVariants?.length ?? 0}): ${row.after.textVariants?.map(quoteText).join(' / ') ?? '(none)'}`,
        );
      }
...
function humanCategory(c: MigrationRowCategory): string {
  switch (c) {
    case 'collapsed':
...
function shortHash(h: string): string {
  if (!h) return '________';
  return h.slice(0, 8);
}

function quoteText(t: string): string {
