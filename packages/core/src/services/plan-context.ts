...
import { getActiveTopics, type ActiveTopicEntry } from '../models/active-topics.js';
import {
  listActiveProjects,
  selectProjectDocs,
  assembleProjectWhatsNew,
  loadMeetingIndex,
 ...
 */
export function extractOpenQuestions(body: string): string[] {
  ...
  for (const raw of lines) {
    ...
    items.push(cleaned);
    if (items.length >= OPEN_QUESTIONS_CAP) break;
  }
  return items;
}

/** Map a ProjectDocSelection into the tagged, flattened bundle doc list. */
function toSelectedDocs(slug: string, sel: ProjectDocSelection): PlanContextSelectedDoc[] {
  const docs: PlanContextSelectedDoc[] = [];
...
function toWhatsNew(wn: ProjectWhatsNew | null): PlanContextWhatsNew | null {
  if (!wn) return null;
  return {
    since: wn.since ?? null,
...
export async function resolveTodayAreas(
  deps: Pick<AssemblePlanContextDeps, 'storage'>,
  ...
  for (const m of index) {
    if (m.date === today && m.area) areas.add(m.area);
  }
  return [...areas].sort();
}
...
export async function assemblePlanContext(
  ...
  let reason: PlanContextBundle['reason'];
  ...
  if (mode === 'project') {
    ...
    const todayAreas =
      opts.todayAreas ?? (await resolveTodayAreas(deps, paths, referenceDate));
  ...
  const ranked: Array<{ slug: string; status?: string; mtimeMs: number }> = [];
  for (const p of candidates) {
    const mod = await deps.storage.getModified(p.readmePath);
    ranked.push({ slug: p.slug, status: p.status, mtimeMs: mod ? mod.getTime() : 0 });
  }
  ranked.sort(
    (a, b) => b.mtimeMs - a.mtimeMs || (a.slug < b.slug ? -1 : a.slug > b.slug ? 1 : 0),
  );
  ...
  for (const c of chosen) {
    ...
    let selection: ProjectDocSelection | null = null;
    try {
    ...
    if (selection) {
      for (const d of selection.expanded) {
        for (const q of extractOpenQuestions(d.body)) {
          ...
          if (openQuestions.length >= OPEN_QUESTIONS_CAP) break;
        }
        if (openQuestions.length >= OPEN_QUESTIONS_CAP) break;
      }
    }
    let whatsNew: PlanContextWhatsNew | null = null;
    try {
      whatsNew = toWhatsNew(
        await assembleProjectWhatsNew(c.slug, paths, {
          storage: deps.storage,
  ...
  const topics: PlanContextTopic[] = [];
  try {
    const { topics: allTopics } = await deps.topicMemory.listAll(paths);
    let activeTopicEntries: ActiveTopicEntry[] = getActiveTopics(allTopics, {
      today: referenceDate,
    });
