...
function classifyIndicator(daysSinceLastMet: number | null): HealthIndicator {
  if (daysSinceLastMet === null) return 'dormant';
  if (daysSinceLastMet <= ACTIVE_THRESHOLD_DAYS) return 'active';
  if (daysSinceLastMet <= REGULAR_THRESHOLD_DAYS) return 'regular';
  if (daysSinceLastMet <= COOLING_THRESHOLD_DAYS) return 'cooling';
  return 'dormant';
}
...
export function computeRelationshipHealth(
  ...
  const parsed = meetingDates
    ...
    .filter((entry): entry is { str: string; date: Date } => entry.date !== null)
  ...
  for (const entry of parsed) {
    ...
    if (days <= 30) meetingsLast30Days++;
    if (days <= 90) meetingsLast90Days++;
  }
