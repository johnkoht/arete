 ...
 */
function computeEntryId(type: string, statement: string, created: string): string {
 ...
 */
function parseFile(content: string | null): ParsedFile {
 ...
 */
function serializeFile(week: string, entries: WeekMemoryEntry[]): string {
 ...
 */
export async function readWeekMemory(
  storage: StorageAdapter,
  root: string,
 ...
 */
export async function listWeekMemory(
  storage: StorageAdapter,
  root: string,
  opts: { active?: boolean } = {},
): Promise<WeekMemoryEntry[]> {
  const entries = await readWeekMemory(storage, root);
  if (opts.active === undefined) return entries;
  const want: WeekMemoryStatus = opts.active ? 'active' : 'resolved';
 ...
 */
export async function addWeekMemoryEntry(
  storage: StorageAdapter,
  root: string,
  input: AddWeekMemoryInput,
  clock: Clock = defaultClock,
): Promise<AddWeekMemoryResult> {
  const content = await storage.read(join(root, WEEK_MEMORY_FILE));
 ...
 */
export async function resolveWeekMemory(
  storage: StorageAdapter,
  root: string,
  id: string,
  clock: Clock = defaultClock,
): Promise<ResolveWeekMemoryResult> {
  const content = await storage.read(join(root, WEEK_MEMORY_FILE));
  ...
  const updated: WeekMemoryEntry = { ...target, status: 'resolved' };
  const next = [...entries];
  next[idx] = updated;
 ...
 */
export async function archiveWeekMemory(
  storage: StorageAdapter,
  root: string,
  clock: Clock = defaultClock,
): Promise<ArchiveWeekMemoryResult> {
