...
export type ResolutionDecisionAction =
  | 'RESOLVE-HIGH-STAGED'
  | 'RESOLVE-HIGH-AUTO'
  | 'RESOLVE-MEDIUM-FLAGGED'
  | 'RESOLVE-USER-CONFIRMED'
  | 'RESOLVE-DEFERRED-TO-FOLLOWUP-2'
  | 'UNRESOLVE'
  | 'UNRESOLVE-PERMANENT'
  | 'UNCONFIRM'
  | 'SUPPRESS-HIT';
 ...
 * this log by id.
 */
export function renderResolutionDecisionLine(
  iso: string,
  payload: ResolutionDecisionLogPayload,
): string {
  return [
    iso,
    payload.action,
 ...
 */
export async function appendResolutionDecisionLog(
...
// ---------------------------------------------------------------------------
// Parser — for `arete resolve --explain` (11-audit) + M4 repeat-detection
// ---------------------------------------------------------------------------
 ...
 */
export function parseResolutionLog(content: string): ResolutionLogEntry[] {
  const out: ResolutionLogEntry[] = [];
  for (const rawLine of content.split(/\r?\n/)) {
    const line = rawLine.trim();
    if (!line) continue;
    // <ISO> <ACTION> phase=<p> <id> <conf> <evidence> <reasoning...>
    const m = line.match(
      /^(\S+)\s+([A-Z0-9-]+)\s+phase=(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s*(.*)$/,
    );
    if (!m) continue;
    out.push({
      iso: m[1],
      action: m[2] as ResolutionDecisionAction,
      phase: m[3],
      id: m[4],
      confidence: m[5],
      evidenceRef: m[6],
 ...
 */
export function hasPriorUnresolveForEvidence(
  entries: ReadonlyArray<ResolutionLogEntry>,
  id: string,
  evidenceRef: string,
  now: Date = new Date(),
  windowDays = 30,
  ...
  for (const e of entries) {
    if (e.action !== 'UNRESOLVE' && e.action !== 'UNRESOLVE-PERMANENT') continue;
    // Match on id (prefix-tolerant) AND evidence.
    if (e.id !== id && e.id.slice(0, 8) !== idPrefix) continue;
    if (e.evidenceRef !== evidenceRef) continue;
    const t = new Date(e.iso).getTime();
    if (Number.isNaN(t)) continue;
    if (now.getTime() - t <= windowMs && now.getTime() - t >= 0) return true;
  }
  return false;
}
