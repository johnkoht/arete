...
const GAP_SUGGESTIONS: Record<ProductPrimitive, string> = {
  ...
  async getRelevantContext(request: ContextRequest): Promise<ContextBundle> {
    ...
    if (contextDirExists) {
      ...
      for (const filePath of contextFiles) {
        ...
        if (relPath.includes('_history')) continue;
        // Skip root-level context files (already handled by PRIMITIVE_FILE_MAP)
        const pathParts = relPath.split('/');
        if (pathParts.length <= 2) continue; // Only process nested files (context/{slug}/*.md or deeper)
        const content = await safeRead(filePath);
        if (content && hasTokenOverlap(content, queryTokens)) {
    ...
    try {
      ...
      for (const result of searchResults) {
        ...
        else if (relPath.startsWith('areas/')) category = 'context'; // Areas use context category
