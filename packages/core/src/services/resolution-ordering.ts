...
export type OrderingDecision =
  | { path: 'auto-resolve'; reason: 'already-committed' }
  | {
 ...
 */
export function decideResolutionOrdering(
  commitmentId: string,
  stillStagedIds: ReadonlySet<string>,
  gmailThreadId: string,
  existingEvidence?: string,
): OrderingDecision {
  const staged =
    stillStagedIds.has(commitmentId) || stillStagedIds.has(commitmentId.slice(0, 8));
  if (!staged) {
