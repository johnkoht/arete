...
import type { Commitment, ExternalSource } from '../models/index.js';
import {
...
const DIRECTIVE_PATTERN =
  /\[\[(confirm|unconfirm|unresolve)\s+([a-fA-F0-9]{8,64}|[a-z0-9][a-z0-9-]{2,})(\s+--permanent)?\]\]/gi;
 ...
 */
export function parseResolutionDirectives(content: string): ParseDirectivesResult {
  const rejectedBulk: RejectedBulkDirective[] = [];
  ...
  while ((bm = BULK_PATTERN.exec(content)) !== null) {
    rejectedBulk.push({
      raw: bm[0],
      message:
        `[[${bm[1]}]] is not supported. Bulk confirm was removed (passive-vote ` +
        `foot-gun). Confirm each entry individually: \`[[confirm <id>]]\`.`,
    });
  }

  const directives: ResolutionDirective[] = [];
  DIRECTIVE_PATTERN.lastIndex = 0;
  let m: RegExpExecArray | null;
  while ((m = DIRECTIVE_PATTERN.exec(content)) !== null) {
    const kind = m[1].toLowerCase() as ResolutionDirectiveKind;
...
// ---------------------------------------------------------------------------
// Mutators — pure Commitment → Commitment (or a no-op result)
// ---------------------------------------------------------------------------

export type MutatorResult =
  | { ok: true; commitment: Commitment; note?: string }
 ...
 */
export function stageResolve(
  commitment: Commitment,
  evidence: { url: string; threadId: string },
  now: Date = new Date(),
): Commitment {
  return {
    ...commitment,
...
export function autoResolve(
  commitment: Commitment,
  evidence: { url: string; threadId: string; sentAt: string },
): Commitment {
 ...
 */
export function applyConfirm(commitment: Commitment, now: Date = new Date()): MutatorResult {
 ...
 */
export function applyUnconfirm(commitment: Commitment, now: Date = new Date()): MutatorResult {
 ...
 */
export function applyUnresolve(
  commitment: Commitment,
  opts: { permanent?: boolean; promoteToPermanent?: boolean; now?: Date } = {},
): MutatorResult {
...
// ---------------------------------------------------------------------------
// First-week promotion gate (F2/AC2a)
// ---------------------------------------------------------------------------
...
function addGmailSource(
  existing: ExternalSource[] | undefined,
  evidence: { url: string; threadId: string },
): ExternalSource[] {
