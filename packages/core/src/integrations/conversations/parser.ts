...
function tryTimestamped(lines: string[]): ParsedConversation | null {
...
function tryStructured(lines: string[]): ParsedConversation | null {
  const messages: ParsedMessage[] = [];
  let currentMessage: ParsedMessage | null = null;
...
function parseRaw(text: string): ParsedConversation {
  ...
  const messages: ParsedMessage[] = paragraphs.map((p) => ({
    speaker: 'Unknown',
    text: p.trim(),
  }));
...
function uniqueParticipants(messages: ParsedMessage[]): string[] {
...
function formatMessages(messages: ParsedMessage[], includeTimestamp: boolean): string {
  return messages
    .map((m) => {
