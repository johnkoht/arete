...
import type { StagedItem } from '../models/index.js';
import {
  renderItemLine,
  isUncertain,
  isOthersAction,
...
import {
  ...
  type ThemeCluster,
} from './winddown-theme-cluster.js';
...
function isSuperseded(meta: ChecklistItemMeta | undefined): boolean {
...
): ThemeLineDecoration | undefined {
  const superseded = isSuperseded(meta);
  const sourceTag = meeting.label && meeting.label.trim() !== ''
    ? `*(${meeting.label.trim()})*`
    : undefined;
  if (!superseded && !sourceTag) return undefined;
  return { superseded, sourceTag };
}

// ---------------------------------------------------------------------------
// Section rendering (consolidated across a theme's meetings, chronological)
// ---------------------------------------------------------------------------

type SectionKind = 'actionItems' | 'decisions' | 'learnings';
...
function renderThemeSection(
  ...
  for (const input of meetings) {
    ...
    for (const item of ordered) {
      ...
      if (opts.isAction && isOthersAction(im)) {
        ...
        fyiLines.push(
          renderItemLine(item, m.slug, im, { isAction: true, forceUnchecked: true, theme: deco }),
        );
      } else {
  ...
  if (lines.length === 0 && fyiLines.length === 0) return '';
  const parts: string[] = [];
  if (lines.length > 0) parts.push(`**${heading}**\n${lines.join('\n')}`);
  if (fyiLines.length > 0) parts.push(`_Others' actions (FYI)_\n${fyiLines.join('\n')}`);
  return parts.join('\n\n');
}

/** Render one project/area (or Uncategorized) theme group. */
function renderThemeGroup(group: ThemeRenderGroup): string {
  ...
  if (group.uncategorized) {
    ...
    blocks.push(
      group.reasoning && group.reasoning.trim() !== ''
  ...
  if (group.uncategorized) {
    ...
    for (const b of [decisions, actions, learnings]) if (b !== '') blocks.push(b);
  }
 ...
 */
export function renderThemeView(view: ThemeView): string {
  ...
  const parts: string[] = [`# Daily Winddown — ${dateLabel} (theme view)`, THEME_LEGEND];
...
// ---------------------------------------------------------------------------
// View builder — clusters → ThemeView (Uncategorized ALWAYS materialized)
// ---------------------------------------------------------------------------
...
export interface BuildThemeViewOptions {
  ...
  themeMeta?: Record<
    string,
    { heading?: string; headingPrefix?: string; reasoning?: string; callout?: string }
  >;
...
export function buildThemeView(
  inputs: ThemeMeetingInput[],
  opts: BuildThemeViewOptions,
): ThemeView {
  const { clusters } = clusterMeetingsByTheme(inputs);
  const themeMeta = opts.themeMeta ?? {};

  const groups: ThemeRenderGroup[] = [];
  let sawUncategorized = false;
  for (const cluster of clusters) {
    if (cluster.uncategorized) sawUncategorized = true;
  ...
  if (!sawUncategorized) {
    groups.push(
      toRenderGroup(
        { theme: UNCATEGORIZED_THEME, uncategorized: true, meetings: [] },
        themeMeta[UNCATEGORIZED_THEME],
      ),
    );
  }
...
function toRenderGroup(
  ...
  meta?: { heading?: string; headingPrefix?: string; reasoning?: string; callout?: string },
): ThemeRenderGroup {
  return {
    theme: cluster.theme,
 ...
 * skipped.
 */
export function pickDominantTheme(
