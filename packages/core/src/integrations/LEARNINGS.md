...
- **Direction-arrow encoding convention** (2026-04-09): `→` = `i_owe_them`, `←` = `they_owe_me` (relative to workspace owner). This convention spans multiple files: `formatActionItemWithOwner()` writes it, `parseOwnerFromText()` reads it for staged items, `APPROVED_OWNER_PATTERN` reads it for approved items in reconciliation, and `ARROW_VARIANTS` in `meeting-parser.ts` must include both arrows. When adding new arrow-like patterns, update ALL these locations. The left slug is always the owner/debtor, the right slug is always the counterparty/creditor — position-based semantics are consistent regardless of arrow direction.
...
## Winddown render / theme-render (2026-06-22)

The winddown-approval stack (single-pass → chef reconcile → checklist/theme render → anchor-keyed apply) lives here (`winddown-checklist.ts`, `winddown-theme-render.ts`, `winddown-theme-cluster.ts`, `staged-items.ts`). Reusable lessons:

- **Two render layouts stay diff-compatible by sharing one anchor emitter.** `checklist` (per-meeting) and `theme` (per-project/area) render the same items with different grouping, but both route through a single `renderItemLine` — the ONLY place the `<!-- id@meeting-slug -->` anchor is written. Because `winddown-apply` diffs purely by anchor (grouping-agnostic), a theme-grouped doc apply-diffs identically to a checklist doc. Lesson: when two layouts must stay diff-compatible, factor the load-bearing token into one shared emitter and decorate only the *visible* text around it — never re-emit the anchor in the second layout.
- **Anchor a "must never happen" invariant to persisted state, not caller discipline.** A superseded item must never render pre-checked `[x]` (the bug it exists to kill: a reversed morning decision got auto-committed). First impl forced `[ ]` only when the *caller* passed a `theme.superseded` flag → latent footgun (a checklist-mode caller would still render `[x]`). Fix: `prefillChecked()` returns false when `meta.skipKind === 'superseded'`, so the invariant holds on the stored signal regardless of render path. Encode safety invariants on the data, not on every call site remembering a flag.
- **Overloading a metadata field needs an explicit discriminator, default-absent = original meaning.** `staged_item_skip_reason` was reused for both dedup ("already captured as X") and supersession ("superseded by X"); without a discriminator the renderer mislabeled superseded items as dedup. Added `kind?: 'dedup' | 'superseded'` — absent ⇒ dedup, so existing skip entries render byte-identically (protects the no-regression invariant). When a field gains a second meaning, add an explicit `kind` and make absent default to the legacy meaning.
- **Count-conservation as a structural throw, not a hope.** The theme clusterer asserts `itemsIn === itemsOut` and throws on mismatch, so a regression can't silently drop or duplicate a staged item. Caveat: the counter sums the three known staged kinds (actions/decisions/learnings) — a new kind must be added to the counter or it would be silently uncounted on both sides.
