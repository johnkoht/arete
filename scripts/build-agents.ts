...
/**
 * Check if root AGENTS.md has been accidentally overwritten with generated content.
 * Root AGENTS.md should be hand-written BUILD-mode content, not generated GUIDE content.
 */
async function checkRootAgentsIntegrity(): Promise<void> {
  const rootAgentsPath = join(root, 'AGENTS.md');
  
  if (!existsSync(rootAgentsPath)) {
    console.warn('⚠ Warning: Root AGENTS.md not found');
    return;
  }
  
  const agentsContent = await readFile(rootAgentsPath, 'utf-8');
  
  // Check for generated header (should NOT be present in hand-written BUILD AGENTS.md)
  const hasGeneratedHeader = agentsContent.includes('<!-- Generated by build-agents.ts');
  
  // Check for GUIDE-specific source references (should NOT be present in BUILD AGENTS.md)
  const hasGuideSourceRefs = agentsContent.includes('guide/skills-index.md') || 
                             agentsContent.includes('guide/tools-index.md');
  
  // Check for BUILD-specific markers (SHOULD be present in BUILD AGENTS.md)
  const hasBuildMarkers = agentsContent.includes('[Identity]|You are the planner') ||
                          agentsContent.includes('[Expertise]|domain knowledge') ||
                          agentsContent.includes('system awareness for BUILD mode');
  
  if (hasGeneratedHeader && hasGuideSourceRefs) {
    console.error('');
    console.error('❌ INTEGRITY CHECK FAILED: Root AGENTS.md contains GUIDE-mode generated content!');
    console.error('   Root AGENTS.md should be hand-written BUILD-mode content.');
    console.error('   It appears to have been overwritten with dist/AGENTS.md content.');
    console.error('');
    console.error('   To fix:');
    console.error('   1. Restore from git: git show cd640c4:AGENTS.md > AGENTS.md');
    console.error('   2. Or manually update AGENTS.md with BUILD-specific content');
    console.error('   3. See .pi/APPEND_SYSTEM.md for BUILD AGENTS.md structure');
    console.error('');
    process.exit(1);
  }
  ...
  if (!hasBuildMarkers) {
    console.warn('');
    console.warn('⚠ Warning: Root AGENTS.md may be missing BUILD-mode markers.');
    console.warn('  Expected [Identity], [Expertise], or BUILD mode description.');
    console.warn('');
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    // Safety check: ensure root AGENTS.md hasn't been overwritten with generated content
    await checkRootAgentsIntegrity();
