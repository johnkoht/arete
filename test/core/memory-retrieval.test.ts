...
describe('memory-retrieval', () => {
  ...
  describe('searchMemory', () => {
    it('returns empty results for empty query tokens', () => {
      const result = searchMemory('the a', paths);
      assert.equal(result.results.length, 0);
      assert.equal(result.total, 0);
    });

    it('returns empty results when memory dir does not exist', () => {
      const result = searchMemory('onboarding', paths);
      assert.equal(result.results.length, 0);
    });

    it('returns empty results when no matching sections', () => {
      writeMemoryFile(tmpDir, 'decisions.md', '# Decisions\n\n### 2026-01-15: Use PostgreSQL\n\n**Decision**: We chose PostgreSQL for the database.\n');
      const result = searchMemory('onboarding', paths);
      assert.equal(result.results.length, 0);
    });

    it('finds decisions matching query tokens', () => {
      writeMemoryFile(tmpDir, 'decisions.md', `# Decisions
...
`);
    ...
    it('finds learnings matching query tokens', () => {
...
`);
      const result = searchMemory('concise summaries', paths);
      assert.ok(result.results.length >= 1);
...
`);
      const result = searchMemory('search', paths);
      assert.ok(result.results.length >= 1);
      // The first result should be the one with "search" in the title
      assert.ok(result.results[0].content.includes('Choose search technology'));
    });

    it('filters by memory type when specified', () => {
      writeMemoryFile(tmpDir, 'decisions.md', '# Decisions\n\n### 2026-01-15: Search decision\n\n**Decision**: Use Elasticsearch.\n');
      writeMemoryFile(tmpDir, 'learnings.md', '# Learnings\n\n### 2026-02-01: Search insight\n\n**Insight**: Users want instant search.\n');
      const result = searchMemory('search', paths, { types: ['learnings'] });
...
`);
      const result = searchMemory('search', paths, { limit: 2 });
      assert.equal(result.results.length, 2);
      assert.equal(result.total, 3);
    });

    it('includes relevance explanation', () => {
      writeMemoryFile(tmpDir, 'decisions.md', '# Decisions\n\n### 2026-01-15: Onboarding redesign\n\n**Decision**: Redesign onboarding.\n');
      const result = searchMemory('onboarding redesign', paths);
      assert.ok(result.results.length >= 1);
      assert.ok(result.results[0].relevance.length > 0);
      assert.ok(result.results[0].relevance.includes('Title matches'));
    });

    it('parses dates from section headings', () => {
      writeMemoryFile(tmpDir, 'decisions.md', '# Decisions\n\n### 2026-02-07: Recent decision\n\n**Decision**: Something.\n');
      const result = searchMemory('recent decision', paths);
      assert.ok(result.results.length >= 1);
      assert.equal(result.results[0].date, '2026-02-07');
    });

    it('handles sections without dates', () => {
      writeMemoryFile(tmpDir, 'learnings.md', '# Learnings\n\n### Important search insight\n\n**Insight**: Search is crucial.\n');
      const result = searchMemory('search insight', paths);
      assert.ok(result.results.length >= 1);
      assert.equal(result.results[0].date, undefined);
    });

    it('searches across multiple memory types by default', () => {
