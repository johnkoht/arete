...
**Staleness caveat (project-agent re-review, 2026-06-23):** if the cached payload ever carries the grounded bundle's *live* fields (Jira `{title,status,owner}`, supersession reasoning), max-mtime keying is unsafe — a server-side ticket change moves no file mtime, so the cache would re-serve the exact stale-ticket fact the grounding exists to fix. If built: cache only file-derived/deterministic fields under mtime keying and **always re-verify `tickets[]` live each run**; no TTL ceiling. Required test: a changed-ticket scenario is NOT served from cache. (The shipped inline build sidesteps this — `now/.cache/agenda-grounding/<slug>.md` is a per-run transient artifact, regenerated every run, never an mtime-keyed persistent cache.)
...
### 7. Spawn-fan-out for agenda grounding (deferred from project-agent-meeting-prep WS-E)
The grounded disposition shipped **inline** (one project grounded per run, dedup-by-project, no subagent). Spawn-fan-out (one subagent per unique project, parallel, isolated prime context) was evaluated at both Phase-1 gates (2026-06-23) and **descoped as over-built** for the real cadence (~3 meetings once/day): isolation's only unique benefit is wall-clock parallelism, which the plan explicitly doesn't need; dedup-by-project works inline with a local loop; and the disposition — not the topology — is what fixes the three errors. Revisit ONLY if batch size or synthesis wall-clock / prime context-size demonstrably hurts. If pursued: (a) prove it with a throwaway spike first — the prime must reliably spawn the grounding subagent from skill prose AND the bundle must round-trip via the disk artifact and demonstrably reach the agenda; (b) prefer a real `.claude/agents/` agentType over prose-spawning; (c) keep F3/AC1 fully in force for prime synthesis (grounding isolation does NOT isolate synthesis). Gate artifacts: `dev/work/plans/project-agent-meeting-prep/{pre-mortem,review}.md`; the fan-out-path artifacts are preserved in main history at `a99cb05a`.
