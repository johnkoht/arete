...
- Fix `files` array: ensure it includes all compiled output and runtime assets
  - **Remove `dist/` from `files`** — root `dist/` is gitignored, never auto-generated by `npm run build`, and would be absent on a clean checkout publish. It's a stale leftover. `packages/runtime/` is the correct source of truth and is already in `files`.
