...
| **AC6** (GATE — prose-pinned, soak-verified) | **`/project` presentation prose: siblings + wiki always shown.** `project/SKILL.md` presentation step: ALWAYS render the Sibling-projects and Related-wiki-pages sections when present in CLI output — never drop as "secondary"; when absent from CLI output, say so in one line ("No siblings / no wiki pages matched"). | Skill-prose test in `chef-orchestrator-skills.test.ts` asserts the always-show rule text. **(Review finding 4, honest label: this is a string-presence assertion, not a behavior test — prose pins the rule, soak verifies the agent follows it. Same epistemic stance as phase-14 AC3's verification split.)** |
