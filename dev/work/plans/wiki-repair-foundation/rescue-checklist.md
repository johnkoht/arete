# Rescue checklist — your call on the 40 archive candidates

**How to use:** check a box (`[x]`) = **KEEP as a topic** (page stays put; costs ~nothing — see below).
Unchecked = archive to `.arete/archive/topics/` (fully reversible via ledger; one `mv` to restore).
Evidence per row = why the instrument thinks it's dead (zero taggable sources, zero body mentions,
zero project refs since the 4/24 seed). Remember the instrument's limits: the corpus lags reality
(your 6/09 1:1 isn't filed yet) and it measures ACTIVITY, not IMPORTANCE — `engineer-autonomy` is
the proof. When in doubt, keep.

## Archive-HIGH candidates (26)
...
- [x] `compliance-admin-panel` — 0 taggable sources, 1 live in-links (4 total), seed=1, soft signals 0; body mentions 0; project refs 0 (seed: 2026-04-13-kim-john-compliance-letters-emails)
...
- [x] `escalations-tracking` — 0 taggable sources, 1 live in-links (3 total), seed=1, soft signals 0; body mentions 0; project refs 0 (seed: 2026-03-30-escalations-email-comms-sync)
- [x] `feedback-collection` — 0 taggable sources, 0 live in-links (4 total), seed=2, soft signals 0; body mentions 0; project refs 0 (seed: 2026-03-05-john-jamie (+1 more))
- [x] `guide-rollout` — 0 taggable sources, 0 live in-links (2 total), seed=1, soft signals 0; body mentions 0; project refs 0 (seed: 2026-02-24-connect-glance-user-guides)
...
- [x] `letter-vendor-evaluation` — 0 taggable sources, 0 live in-links (5 total), seed=1, soft signals 2; body mentions 0; project refs 0 (seed: 2026-04-09-smartcomms-demo)
...
- [x] `pop-adjuster-workflow` — 0 taggable sources, 0 live in-links (4 total), seed=1, soft signals 1; body mentions 0; project refs 0 (seed: 2026-04-08-pop-snapsheet-functionality-review)
...
- [x] `smartcoms-integration` — 0 taggable sources, 1 live in-links (3 total), seed=1, soft signals 0; body mentions 0; project refs 0 (seed: 2026-03-26-cover-whale-email-templaes-sync)
- [x] `snapsheet-file-note-sync` — 0 taggable sources, 1 live in-links (4 total), seed=1, soft signals 1; body mentions 0; project refs 0 (seed: 2026-03-23-glance-emails-prod-access)
- [x] `snapsheet-task-replacement` — 0 taggable sources, 0 live in-links (5 total), seed=1, soft signals 0; body mentions 0; project refs 0 (seed: 2026-04-08-glance-mvp-weekly)
...
- [x] `workflow-tool-integration` — 0 taggable sources, 0 live in-links (3 total), seed=1, soft signals 0; body mentions 0; project refs 0 (seed: 2026-03-17-pop-glance-emails)

## Archive-MEDIUM candidates (14)
- [x] `ai-claims-automation` — 0 taggable sources, seed=1, live in-links 0, soft signals 5 (0 live-dup / 5 weak / 0 title-only); body mentions 0; project refs 0
...
- [x] `default-recipients` — 0 taggable sources, seed=4, live in-links 2, soft signals 0 (0 live-dup / 0 weak / 0 title-only); body mentions 0; project refs 0
...
- [x] `engineer-autonomy` — 0 taggable sources, seed=1, live in-links 2, soft signals 0 (0 live-dup / 0 weak / 0 title-only); body mentions 0; project refs 0
...
- [x] `template-admin-page` — 0 taggable sources, seed=3, live in-links 3, soft signals 0 (0 live-dup / 0 weak / 0 title-only); body mentions 0; project refs 0
...
## Hand-review items (separate from the checklist — see rescue-proposal-v2.md for detail)
- 3 per-pair merges (§"Merges into a LIVE partner page"): `claim-transfer-tdd`→`tdd`, `email-rollout-strategy`→`rollout-strategy`, `fraud-language`→`doi-fraud-language`
- 13 alias conflicts in the starved-taggable band (§"starved-taggable") — same alias claimed by two canonicals; pick one each
- `email-templates` direction call: also absorb the live `default-email-template` (~$0.43 duplication) or keep two pages

## What "keep" actually costs (for calibration)
- **Money/perf: zero.** Frozen pages are no-ops in refresh walks; no LLM spend; disk trivial.
- **Lint nag: capped.** One stale topic per winddown, regardless of how many are stale.
- **Retrieval: labeled, minor.** Stale pages can appear in search/briefs but now carry
  "(as of 2026-04-24 — stale)" labels; worst case they occupy a ranking slot a live page wanted.
- **Dormancy is graceful now:** past 90d they fall off the extraction bias list (hibernate); if a
  topic revives, mentions surface as orphan tags via the chef's nudge → restore/refresh is trivial.
Archive is curation hygiene, not resource savings. Bias to keep is sound.
